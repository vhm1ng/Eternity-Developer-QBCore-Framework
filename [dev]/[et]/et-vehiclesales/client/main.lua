        ...
        if vehicles then
            for i = 1, #vehicles, 1 do
                ...
                if Config.UseTarget then
                    ...
                    EntityZones[i] = exports['et-target']:AddTargetEntity(occasionVehicles[Zone][i].car, {
                        options = {
                            {
                                type = "client",
                                event = "et-vehiclesales:client:OpenContract",
                                icon = "fas fa-car",
...
local function despawnOccasionsVehicles()
    ...
    for i = 1, #oSlot, 1 do
        ...
        if EntityZones[i] and Config.UseTarget then
            exports['et-target']:RemoveZone(EntityZones[i])
        end
    end
    EntityZones = {}
end
...
local function SellData(data, model)
    QBCore.Functions.TriggerCallback("et-vehiclesales:server:CheckModelName",function(DataReturning)
        local vehicleData = {}
        ...
        TriggerServerEvent('et-occasions:server:sellVehicle', data.price, vehicleData)
        ...
        while listen do
            if IsControlJustReleased(0, 38) then -- E
                if spot then
                    ...
                    TriggerEvent('et-vehiclesales:client:OpenContract', data)
                else
                    if IsPedInAnyVehicle(PlayerPedId(), false) then
                        listen = false
                        TriggerEvent('et-occasions:client:MainMenu')
                        --TriggerEvent('et-vehiclesales:client:SellVehicle')
                    else
    ...
    for k, v in pairs(Config.Zones) do
        ...
        SellSpot:onPlayerInOut(function(isPointInside)
            if isPointInside and Zone ~= k then
                Zone = k
                QBCore.Functions.TriggerCallback('et-occasions:server:getVehicles', function(vehicles)
...
RegisterNUICallback('buyVehicle', function(_, cb)
    TriggerServerEvent('et-occasions:server:buyVehicle', CurrentVehicle)
    cb('ok')
end)

RegisterNUICallback('takeVehicleBack', function(_, cb)
    TriggerServerEvent('et-occasions:server:ReturnVehicle', CurrentVehicle)
    cb('ok')
end)

-- Events

RegisterNetEvent('et-occasions:client:BuyFinished', function(vehdata)
    ...
    QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
        ...
        TriggerEvent("vehiclekeys:client:SetOwner2", vehdata.plate)
...
RegisterNetEvent('et-occasions:client:SellBackCar', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        QBCore.Functions.TriggerCallback('et-garage:server:checkVehicleOwner', function(owned, balance)
            if owned then
                if balance < 1 then
                    TriggerServerEvent('et-occasions:server:sellVehicleBack', vehicleData)
...
RegisterNetEvent('et-occasions:client:ReturnOwnedVehicle', function(vehdata)
    local vehmods = json.decode(vehdata.mods)
    DoScreenFadeOut(250)
    Wait(500)
    QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
...
RegisterNetEvent('et-occasion:client:refreshVehicles', function()
    if Zone then
        QBCore.Functions.TriggerCallback('et-occasions:server:getVehicles', function(vehicles)
            despawnOccasionsVehicles()
            spawnOccasionsVehicles(vehicles)
...
RegisterNetEvent('et-vehiclesales:client:SellVehicle', function()
...
RegisterNetEvent('et-vehiclesales:client:OpenContract', function(data)
    CurrentVehicle = occasionVehicles[Zone][data.Contract]
    if CurrentVehicle then
        QBCore.Functions.TriggerCallback('et-occasions:server:getSellerInformation', function(info)
            if info then
...
RegisterNetEvent('et-occasions:client:MainMenu', function()
    local MainMenu = {
        {
            isMenuHeader = true,
        ...
        },
        {
            ...
            params = {
                event = 'et-vehiclesales:client:SellVehicle',
            }
        },
        {
            ...
            params = {
                event = 'et-occasions:client:SellBackCar',
            }
        }
    }

    exports['et-menu']:openMenu(MainMenu)
end)
    ...
    for k, cars in pairs(Config.Zones) do
        ...
        SpawnZone[k]:onPlayerInOut(function(isPointInside)
            if isPointInside and IsPedInAnyVehicle(PlayerPedId(), false) then
                exports['et-core']:DrawText(Lang:t("menu.interaction"), 'left')
                TextShown = true
                Listen4Control()
            else
                listen = false
            ...
            for k2, v in pairs(Config.Zones[k].VehicleSpots) do
                ...
                VehicleZones:onPlayerInOut(function(isPointInside)
                    if isPointInside and IsCarSpawned(k2) then
                        exports['et-core']:DrawText(Lang:t("menu.view_contract_int"), 'left')
                        TextShown = true
                        Listen4Control(k2)
                    else
                        listen = false

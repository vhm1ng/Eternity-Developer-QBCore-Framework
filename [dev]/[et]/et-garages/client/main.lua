...
function TrackVehicleByPlate(plate)
    QBCore.Functions.TriggerCallback('et-garages:server:GetVehicleLocation', function(coords)
...
function RemoveRadialOptions()
    if MenuItemId1 ~= nil then
        exports['et-radialmenu']:RemoveOption(MenuItemId1)
        MenuItemId1 = nil
    end
    if MenuItemId2 ~= nil then
        exports['et-radialmenu']:RemoveOption(MenuItemId2)
        MenuItemId2 = nil
    end
end
--Menus
local function PublicGarage(garageName, type)
    ...
    exports['et-menu']:openMenu({
        ...
        },
        {
            ...
            params = {
                event = "et-garages:client:GarageMenu",
                args = {
                    garageId = garageName,
                    garage = garage,
...
local function ClearMenu()
	TriggerEvent("et-menu:closeMenu")
end
...
local function ExitAndDeleteVehicle(vehicle)
    ...
    if Config.SpawnVehiclesServerside then
        Wait(1000)
        TriggerServerEvent('et-garages:server:parkVehicle', plate)
    end
end
...
local function ParkOwnedVehicle(veh, garageName, vehLocation, plate)
    ...
    if Config.FuelScript then
        ...
        totalFuel = exports['vhming-fuel']:GetFuel(veh) -- Don't change this. Change it in the  Defaults to legacy fuel if not set in the config
    end
    ...
    TriggerServerEvent('et-garage:server:updateVehicle', 1, totalFuel, engineDamage, bodyDamage, properties, plate, garageName, Config.StoreParkinglotAccuratly and closestVec3 or nil, Config.StoreDamageAccuratly and GetCarDamage(veh) or nil)
    ExitAndDeleteVehicle(veh)
    if plate then
        OutsideVehicles[plate] = nil
        TriggerServerEvent('et-garages:server:UpdateOutsideVehicles', OutsideVehicles)
    end
...
function ParkVehicleSpawnerVehicle(veh, garageName, vehLocation, plate)
    QBCore.Functions.TriggerCallback("et-garage:server:CheckSpawnedVehicle", function (result)
        local canPark, _ = CanParkVehicle(veh, garageName, vehLocation)
        if result and canPark then
            TriggerServerEvent("et-garage:server:UpdateSpawnedVehicle", plate, nil)
            ExitAndDeleteVehicle(veh)
...
local function ParkVehicle(veh, garageName, vehLocation)
    ...
    QBCore.Functions.TriggerCallback('et-garage:server:checkOwnership', function(owned)
        if owned then
...
local function AddRadialParkingOption()
    ...
    if (veh and dist <= Config.VehicleParkDistance and Config.AllowParkingFromOutsideVehicle) or IsPedInAnyVehicle(ped) then
        MenuItemId1 = exports['et-radialmenu']:AddOption({
            id = 'put_up_vehicle',
            title = 'Park Vehicle',
            icon = 'square-parking',
            type = 'client',
            event = 'et-garages:client:ParkVehicle',
            shouldClose = true
        }, MenuItemId1)
    end
    MenuItemId2 = exports['et-radialmenu']:AddOption({
        id = 'open_garage_menu',
        title = 'Open Garage',
        icon = 'warehouse',
        type = 'client',
        event = 'et-garages:client:OpenMenu',
        shouldClose = true
    }, MenuItemId2)
end
...
local function CreateGarageZone()
    ...
    combo:onPlayerInOut(function(isPointInside, l, zone)
        if isPointInside and IsAuthorizedToAccessGarage(zone.name) then
            ...
            exports['et-core']:DrawText(Config.Garages[CurrentGarage]['drawText'], Config.DrawTextPosition)
        else
            CurrentGarage = nil
            RemoveRadialOptions()
...
local function RegisterHousePoly(house)
    ...
    zone:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            exports['et-core']:DrawText(Config.HouseParkingDrawText, Config.DrawTextPosition)
        else
            exports['et-core']:HideText()
            RemoveRadialOptions()
...
function JobMenuGarage(garageName)
    ...
    for veh, label in pairs(vehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "",
            params = {
                event = "et-garages:client:TakeOutGarage",
                args = {
                    vehicleModel = veh,
                    garage = garage
                }
...
local function UpdateVehicleSpawnerSpawnedVehicle(veh, garage, heading, cb)
    ...
    if Config.FuelScript then
        ...
        exports['vhming-fuel']:SetFuel(veh, 100) -- Don't change this. Change it in the  Defaults to legacy fuel if not set in the config
    end
...
function UpdateSpawnedVehicle(spawnedVehicle, vehicleInfo, heading, garage, properties)
    ...
    if garage.useVehicleSpawner then
        ...
        if Config.FuelScript then
            ...
            exports['vhming-fuel']:SetFuel(spawnedVehicle, 100) -- Don't change this. Change it in the  Defaults to legacy fuel if not set in the config
        end
        TriggerEvent("vehiclekeys:client:SetOwner2", plate)
        TriggerServerEvent("et-garage:server:UpdateSpawnedVehicle", plate, true)
    else
        ...
        if Config.FuelScript then
            ...
            exports['vhming-fuel']:SetFuel(spawnedVehicle, vehicleInfo.fuel) -- Don't change this. Change it in the  Defaults to legacy fuel if not set in the config
        end
...
RegisterNetEvent("et-garages:client:GarageMenu", function(data)
    ...
    QBCore.Functions.TriggerCallback("et-garage:server:GetGarageVehicles", function(result)
        if result == nil then
            ...
            for k, v in pairs(result) do
                ...
                if type == "depot" then
                    MenuGarageOptions[#MenuGarageOptions+1] = {
                        ...
                        params = {
                            event = "et-garages:client:TakeOutDepot",
                            args = {
                                vehicle = v,
            ...
            exports['et-menu']:openMenu(MenuGarageOptions)
        end
    end, garageId, type, superCategory)
end)

RegisterNetEvent('et-garages:client:TakeOutGarage', function(data, cb)
    ...
    if garage.useVehicleSpawner then
        ...
        if Config.SpawnVehiclesServerside then
            QBCore.Functions.TriggerCallback('et-garage:server:spawnvehicle', function(netId, properties)
                local veh = NetToVeh(netId)
...
RegisterNetEvent('et-garages:client:OpenMenu', function()
    ...
    elseif CurrentHouseGarage then
        TriggerEvent('et-garages:client:OpenHouseGarage')
    end
end)

RegisterNetEvent('et-garages:client:ParkVehicle', function()
    local ped = PlayerPedId()
    local canPark = true
...
RegisterNetEvent('et-garages:client:ParkLastVehicle', function(parkingName)
    local ped = PlayerPedId()
...
RegisterNetEvent('et-garages:client:TakeOutDepot', function(data)
    local vehicle = data.vehicle
    ...
    if not vehExists then
        ...
        if PlayerData.money['cash'] >= vehicle.depotprice or PlayerData.money['bank'] >= vehicle.depotprice then
            TriggerEvent("et-garages:client:TakeOutGarage", data, function (veh)
                if veh then
                    TriggerServerEvent("et-garage:server:PayDepotPrice", data)
                end
            end)
        else
...
RegisterNetEvent('et-garages:client:TrackVehicleByPlate', function(plate)
    TrackVehicleByPlate(plate)
end)

RegisterNetEvent('et-garages:client:OpenHouseGarage', function()
    MenuHouseGarage()
end)

RegisterNetEvent('et-garages:client:setHouseGarage', function(house, hasKey)
    if hasKey then
...
RegisterNetEvent('et-garages:client:houseGarageConfig', function(garageConfig)
...
RegisterNetEvent('et-garages:client:addHouseGarage', function(house, garageInfo)
...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    ...
    QBCore.Functions.TriggerCallback('et-garage:server:GetOutsideVehicles', function(outsideVehicles)

...
local function MenuGarage()
    ...
    for k in pairs(Config.Vehicles) do
        towMenu[#towMenu+1] = {
            ...
            params = {
                event = "et-tow:client:TakeOutVehicle",
                args = {
                    vehicle = k
                }
            }
    ...
    exports['et-menu']:openMenu(towMenu)
end
...
local function CreateZone(type, number)
    ...
    elseif type == "towspots" then
        event = "et-tow:client:SpawnNPCVehicle"
    ...
    if Config.UseTarget and type == "main" then
        exports['et-target']:AddBoxZone(boxName, coords, size, size, {
            minZ = coords.z - 5.0,
            maxZ = coords.z + 5.0,
            name = boxName,
            heading = heading,
        ...
        zoneCombo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if type == "main" then
                    TriggerEvent('et-tow:client:PaySlip')
                elseif type == "vehicle" then
                    TriggerEvent('et-tow:client:Vehicle')
                elseif type == "towspots" then
                    TriggerEvent('et-tow:client:SpawnNPCVehicle')
                end
            end
        end)
        if type == "vehicle" then
            ...
            zoneComboV:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    TriggerEvent('et-tow:client:ShowMarker', true)
                else
                    TriggerEvent('et-tow:client:ShowMarker', false)
                end
            end)
        elseif type == "towspots" then
...
RegisterNetEvent('et-tow:client:SpawnVehicle', function()
...
RegisterNetEvent('et-tow:client:TowVehicle', function()
                    ...
                    if #(towPos - targetPos) < 11.0 then
                        QBCore.Functions.Progressbar("towing_vehicle", Lang:t("mission.towing_vehicle"), 5000, false, true, {
                            ...
                            if NpcOn then
                                ...
                                TriggerServerEvent('et-tow:server:nano', vehNetID)
                                --remove zone
...
RegisterNetEvent('et-tow:client:TakeOutVehicle', function(data)
    local coords = Config.Locations["vehicle"].coords
    ...
    if #(pos - coords) <= 5 then
        ...
        TriggerServerEvent('et-tow:server:DoBail', true, vehicleInfo)
...
RegisterNetEvent('et-tow:client:Vehicle', function()
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
    if not CurrentTow then
        if vehicle and isTowVehicle(vehicle) then
            DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
            TriggerServerEvent('et-tow:server:DoBail', false)
        else
            MenuGarage()
        end
    else
...
RegisterNetEvent('et-tow:client:PaySlip', function()
    if JobsDone > 0 then
        RemoveBlip(CurrentBlip)
        TriggerServerEvent("et-tow:server:11101110", JobsDone)
        JobsDone = 0
        NpcOn = false
    else
...
RegisterNetEvent('et-tow:client:SpawnNPCVehicle', function()
    if not VehicleSpawned then
        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
            local veh = NetToVeh(netId)
            exports['vhming-fuel']:SetFuel(veh, 0.0)
            VehicleSpawned = true
        end, CurrentLocation.model, CurrentLocation, false)
    end
end)

RegisterNetEvent('et-tow:client:ShowMarker', function(active)
    if PlayerJob.name == "tow" then
        showMarker = active
    end
end)

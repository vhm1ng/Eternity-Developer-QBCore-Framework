...
CreateThread(function()
    if Config.UseTarget then
        for key, value in pairs(Config.PawnLocation) do
            exports['et-target']:AddBoxZone('PawnShop'..key, value.coords, value.length, value.width, {
                name = 'PawnShop'..key,
                ...
                options = {
                    {
                        type = 'client',
                        event = 'et-pawnshop:client:openMenu',
                        icon = 'fas fa-ring',
                        label = 'Pawn Shop',
                    },
                },
...
RegisterNetEvent('et-pawnshop:client:openMenu', function()
        ...
        if GetClockHours() >= Config.TimeOpen and GetClockHours() <= Config.TimeClosed then
            local pawnShop = {
                ...
                },
                {
                    ...
                    params = {
                        event = 'et-pawnshop:client:openPawn',
                        args = {
            ...
            if not isMelting then
                pawnShop[#pawnShop + 1] = {
                    ...
                    params = {
                        event = 'et-pawnshop:client:openMelt',
                        args = {
            ...
            if canTake then
                pawnShop[#pawnShop + 1] = {
                    ...
                    params = {
                        isServer = true,
                        event = 'et-pawnshop:server:pickupMelted',
                        args = {
                            items = meltedItem
                        }
                    }
            ...
            exports['et-menu']:openMenu(pawnShop)
        else
...
RegisterNetEvent('et-pawnshop:client:openPawn', function(data)
    QBCore.Functions.TriggerCallback('et-pawnshop:server:getInv', function(inventory)
            ...
            for i = 1, #data.items do
                if v.name == data.items[i].item then
                    pawnMenu[#pawnMenu + 1] = {
                        ...
                        params = {
                            event = 'et-pawnshop:client:pawnitems',
                            args = {
        ...
        exports['et-menu']:openMenu(pawnMenu)
    end)
end)

RegisterNetEvent('et-pawnshop:client:openMelt', function(data)
    QBCore.Functions.TriggerCallback('et-pawnshop:server:getInv', function(inventory)
        local PlyInv = inventory
        local meltMenu = {
            {
            ...
            for i = 1, #data.items do
                if v.name == data.items[i].item then
                    meltMenu[#meltMenu + 1] = {
                        ...
                        params = {
                            event = 'et-pawnshop:client:meltItems',
                            args = {
                                label = QBCore.Shared.Items[v.name].label,
        ...
        exports['et-menu']:openMenu(meltMenu)
    end)
end)

RegisterNetEvent('et-pawnshop:client:pawnitems', function(item)
    local sellingItem = exports['et-input']:ShowInput({
        header = Lang:t('info.title'),
    ...
    if sellingItem then
        ...
        if tonumber(sellingItem.amount) > 0 then
            TriggerServerEvent('et-pawnshop:server:sellPawnItems', item.name, sellingItem.amount, item.price)
        else
...
RegisterNetEvent('et-pawnshop:client:meltItems', function(item)
    local meltingItem = exports['et-input']:ShowInput({
        header = Lang:t('info.melt'),
    ...
    if meltingItem then
        ...
        if meltingItem.amount ~= nil then
            if tonumber(meltingItem.amount) > 0 then
                TriggerServerEvent('et-pawnshop:server:meltItemRemove', item.name, meltingItem.amount, item)
...
RegisterNetEvent('et-pawnshop:client:startMelting', function(item, meltingAmount, meltTimees)
    if not isMelting then
        isMelting = true
        meltTime = meltTimees
        meltedItem = {}
...
RegisterNetEvent('et-pawnshop:client:resetPickup', function()
    canTake = false
end)

...
local function exploitBan(id, reason)
    ...
    TriggerEvent('et-log:server:CreateLog', 'pawnshop', 'Player Banned', 'red',
        string.format('%s was banned by %s for %s', GetPlayerName(id), 'et-pawnshop', reason), true)
    DropPlayer(id, 'You were permanently banned by the server for: Exploiting')
end

RegisterNetEvent('et-pawnshop:server:sellPawnItems', function(itemName, itemAmount, itemPrice)
    local src = source
    ...
    TriggerClientEvent('et-pawnshop:client:openMenu', src)
end)

RegisterNetEvent('et-pawnshop:server:meltItemRemove', function(itemName, itemAmount, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.Functions.RemoveItem(itemName, itemAmount) then
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[itemName], 'remove')
        local meltTime = (tonumber(itemAmount) * item.time)
        TriggerClientEvent('et-pawnshop:client:startMelting', src, item, tonumber(itemAmount), (meltTime * 60000 / 1000))
...
RegisterNetEvent('et-pawnshop:server:pickupMelted', function(item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local playerCoords = GetEntityCoords(GetPlayerPed(src))
    local dist
    ...
    TriggerClientEvent('et-pawnshop:client:resetPickup', src)
    TriggerClientEvent('et-pawnshop:client:openMenu', src)
end)

QBCore.Functions.CreateCallback('et-pawnshop:server:getInv', function(source, cb)

...
local function DeliverAnim()
    ...
    if Config.UseTarget and hasBag then
        ...
        exports['et-target']:RemoveTargetEntity(garbageVehicle)
        if (amountOfBags - 1) <= 0 then
            ...
            exports['et-target']:AddCircleZone('garbagebin', vector3(CL.coords.x, CL.coords.y, CL.coords.z), 2.0,{
...
function TakeAnim()
    ...
    QBCore.Functions.Progressbar("bag_pickup", Lang:t("info.picking_bag"), math.random(3000, 5000), false, true, {
        ...
        if Config.UseTarget and not hasBag then
            hasBag = true
            exports['et-target']:RemoveZone("garbagebin")
            exports['et-target']:AddTargetEntity(garbageVehicle, {
            options = {
                ...
                if not hasBag and canTakeBag then
                    if Distance < 1.5 then
                        if not GarbText then
                            GarbText = true
                            exports['et-core']:DrawText(Lang:t("info.grab_garbage"), 'left')
                        end
                    ...
                    if DoesEntityExist(garbageVehicle) then
                        ...
                        if TruckDist < 2 then
                            if not TrucText then
                                TrucText = true
                                exports['et-core']:DrawText(Lang:t("info.dispose_garbage"), 'left')
                            end
    ...
    CreateThread(function()
        while ControlListen do
            if IsControlJustReleased(0, 38) then
                TriggerEvent("et-garbagejob:client:MainMenu")
            end
            Wait(1)
        end
    end)
    ...
    for i = 1, #Config.Peds do
        ...
        if Config.UseTarget then
            exports['et-target']:AddTargetEntity(ped, {
                options = {{type = "client", event = "et-garbagejob:client:MainMenu", label = Lang:t("target.talk"), icon = 'fa-solid fa-recycle', job = "garbage",}},
                distance = 2.0
            })
        else
            ...
            if options then
                ...
                zone:onPlayerInOut(function(inside)
                    if LocalPlayer.state.isLoggedIn then
                        if inside then
                            exports['et-core']:DrawText(Lang:t("info.talk"), 'left')
                            Listen4Control()
                        else
                            ControlListen = false
                            exports['et-core']:HideText()
...
RegisterNetEvent('et-garbagejob:client:RequestRoute', function()
...
RegisterNetEvent('et-garbagejob:client:RequestPaycheck', function()
...
RegisterNetEvent('et-garbagejob:client:MainMenu', function()
    local MainMenu = {}
    MainMenu[#MainMenu+1] = {isMenuHeader = true,header = Lang:t("menu.header")}
    MainMenu[#MainMenu+1] = { header = Lang:t("menu.collect"),txt = Lang:t("menu.return_collect"),params = { event = 'et-garbagejob:client:RequestPaycheck',}}
    if not garbageVehicle or finished then
        MainMenu[#MainMenu+1] = { header = Lang:t("menu.route"), txt = Lang:t("menu.request_route"), params = { event = 'et-garbagejob:client:RequestRoute',}}
    end

...
RegisterNetEvent('et-admin:client:inventory', function(targetPed)
    TriggerServerEvent("inventory:server:OpenInventory", "otherplayer", targetPed)
end)

RegisterNetEvent('et-admin:client:spectate', function(targetPed)
...
RegisterNetEvent('et-admin:client:SendReport', function(name, src, msg)
    TriggerServerEvent('et-admin:server:SendReport', name, src, msg)
end)
...
RegisterNetEvent('et-admin:client:SaveCar', function()
    ...
    if veh ~= nil and veh ~= 0 then
        ...
        if QBCore.Shared.Vehicles[vehname] ~= nil and next(QBCore.Shared.Vehicles[vehname]) ~= nil then
            TriggerServerEvent('et-admin:server:SaveCar', props, QBCore.Shared.Vehicles[vehname], GetHashKey(veh), plate)
        else
...
RegisterNetEvent('et-admin:client:SetModel', function(skin)
    local ped = PlayerPedId()
...
RegisterNetEvent('et-admin:client:SetSpeed', function(speed)
    local ped = PlayerId()
    if speed == "fast" then
...
RegisterNetEvent('et-weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    local ped = PlayerPedId()
...
RegisterNetEvent('et-admin:client:GiveNuiFocus', function(focus, mouse)
...
RegisterNetEvent('et-admin:client:maxmodVehicle', function()

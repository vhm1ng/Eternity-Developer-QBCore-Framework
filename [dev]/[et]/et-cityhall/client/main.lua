        ...
        if Config.UseTarget then
            ...
            if current.drivingschool then
                opts = {
                    ...
                    action = function()
                        TriggerServerEvent('et-cityhall:server:sendDriverTest', Config.DrivingSchools[closestDrivingSchool].instructors)
                    end
                }
...
RegisterNetEvent('et-cityhall:client:getIds', function()
    TriggerServerEvent('et-cityhall:server:getIDs')
end)

RegisterNetEvent('et-cityhall:client:sendDriverEmail', function(charinfo)
...
RegisterNUICallback('requestId', function(id, cb)
    ...
    if inRangeCityhall and license and id.cost == license.cost then
        TriggerServerEvent('et-cityhall:server:requestId', id.type, closestCityhall)
...
RegisterNUICallback('applyJob', function(job, cb)
    if inRangeCityhall then
        TriggerServerEvent('et-cityhall:server:ApplyJob', job, Config.Cityhalls[closestCityhall].coords)
    else
        QBCore.Functions.Notify(Lang:t('error.not_in_range'), 'error')
    end
    cb('ok')
...
CreateThread(function()
    initBlips()
    spawnPeds()
    QBCore.Functions.TriggerCallback('et-cityhall:server:receiveJobs', function(result)
        SendNUIMessage({
            action = 'setJobs',
            jobs = result
        })

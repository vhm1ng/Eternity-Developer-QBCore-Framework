    ...
    if looped == false then
        ...
        while true do
            ...
            -- if LocalPlayer.state.isLoggedIn then
                    ...
                    if driver ~= 0 and not IsPedAPlayer(driver) and not HasKeys(plate) and not carIsImmune then
                        if IsEntityDead(driver) then
                            if not isTakingKeys then
                                ...
                                TriggerServerEvent('et-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 1)
                        ...
                        elseif Config.LockNPCDrivingCars then
                            TriggerServerEvent('et-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 2)
                        else
                            TriggerServerEvent('et-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 1)
                            -- TriggerServerEvent('et-vehiclekeys:server:AcquireVehicleKeys', plate)
                ...
                --         end
                --     end
                --     print('Hotwiring while in vehicle, also keeps engine off for vehicles you dont own keys to')
                -- end
...
RegisterNetEvent('vehiclekeys:client:SetOwner2')
AddEventHandler('vehiclekeys:client:SetOwner2', function(plate)
    ...
    HasKey = true
    QBCore.Functions.Notify('Bạn đã nhận được chìa khóa của phương tiện', 'success', 3500)
end)
...
RegisterNetEvent('et-vehiclekeys:client:AddKeys', function(plate)
    KeysList[plate] = true
    local ped = PlayerPedId()
    if IsPedInAnyVehicle(ped, false) then
        local vehicle = GetVehiclePedIsIn(ped)
...
RegisterNetEvent('et-vehiclekeys:client:RemoveKeys', function(plate)
    KeysList[plate] = nil
end)
RegisterNetEvent('et-vehiclekeys:client:ToggleEngine', function()
...
RegisterNetEvent('et-vehiclekeys:client:GiveKeys', function(id)
            ...
            if id and type(id) == "number" then -- Give keys to specific ID
                ...
                if IsPedSittingInVehicle(PlayerPedId(), targetVehicle) then -- Give keys to everyone in vehicle
                    ...
                    for p=1,#otherOccupants do
                        TriggerServerEvent('et-vehiclekeys:server:GiveVehicleKeys', GetPlayerServerId(NetworkGetPlayerIndexFromPed(otherOccupants[p])), targetPlate)
                    end
        ...
        if not isBlacklistedVehicle(veh) then
            if HasKeys(QBCore.Functions.GetPlate(veh)) or AreKeysJobShared(veh) then
                ...
                if vehLockStatus == 1 then
                    TriggerServerEvent('et-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(veh), 2)
                    QBCore.Functions.Notify(Lang:t("notify.vlock"), "primary")
                else
                    TriggerServerEvent('et-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(veh), 1)
...
function GiveKeys(id, plate)
    ...
    if distance < 1.5 and distance > 0.0 then
        TriggerServerEvent('et-vehiclekeys:server:GiveVehicleKeys', id, plate)
    else
        QBCore.Functions.Notify(Lang:t("notify.nonear"),'error')
    end
end
function GetKeys()
    QBCore.Functions.TriggerCallback('et-vehiclekeys:server:GetVehicleKeys', function(keysList)
        KeysList = keysList
    end)
end
            ...
            for _, vehicle in pairs(v.vehicles) do
                if string.upper(vehicle) == string.upper(vehName) then
                    if not HasKeys(vehPlate) then
                        TriggerServerEvent("et-vehiclekeys:server:AcquireVehicleKeys", vehPlate)
                    end
                    return true
                end
            end

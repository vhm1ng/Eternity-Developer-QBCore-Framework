...
RegisterNetEvent('et-atms:client:updateBankInformation', function(banking)
    SendNUIMessage({
...
if Config.UseTarget then
    CreateThread(function()
        exports['et-target']:AddTargetModel(Config.ATMModels, {
            options = {
                {
                    event = 'et-atms:server:enteratm',
                    type = 'server',
...
RegisterNetEvent('et-atms:client:loadATM', function(cards)
...
RegisterNUICallback("doATMWithdraw", function(data)
    if data then
        TriggerServerEvent('et-atms:server:doAccountWithdraw', data)
    end
end)

RegisterNUICallback("loadBankingAccount", function(data)
    QBCore.Functions.TriggerCallback('et-atms:server:loadBankAccount', function(banking)
...
RegisterNUICallback("removeCard", function(data)
    QBCore.Functions.TriggerCallback('et-debitcard:server:deleteCard', function(hasDeleted)
        if hasDeleted then
            SetNuiFocus(false, false)
            SendNUIMessage({
                status = "closeATM"

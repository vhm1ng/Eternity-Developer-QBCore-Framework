...
QBCore.Functions.CreateCallback('et-drugs:server:cornerselling:getAvailableDrugs', function(source, cb)
    cb(getAvailableDrugs(source))
end)

RegisterNetEvent('et-drugs:server:giveStealItems', function(drugType, amount)
...
RegisterNetEvent('et-drugs:server:sellCornerDrugs', function(drugType, amount, price)
    local src = source
    ...
    if hasItem.amount >= amount then
        ...
        TriggerClientEvent('et-drugs:client:refreshAvailableDrugs', src, getAvailableDrugs(src))
    else
        TriggerClientEvent('et-drugs:client:cornerselling', src)
    end
end)

RegisterNetEvent('et-drugs:server:robCornerDrugs', function(drugType, amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local availableDrugs = getAvailableDrugs(src)

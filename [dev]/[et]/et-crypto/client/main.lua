...
local function ExchangeSuccess()
	TriggerServerEvent('et-crypto:server:ExchangeSuccess', math.random(1, 10))
end
...
local function ExchangeFail()
	...
	if RemoveChance == LosingNumber then
		TriggerServerEvent('et-crypto:server:ExchangeFail')
		TriggerServerEvent('et-crypto:server:SyncReboot')
	end
end
	...
	CreateThread(function()
		while Crypto.Exchange.RebootInfo.state do
			if (Crypto.Exchange.RebootInfo.percentage + 1) <= 100 then
				...
				TriggerServerEvent('et-crypto:server:Rebooting', true, Crypto.Exchange.RebootInfo.percentage)
			else
				Crypto.Exchange.RebootInfo.percentage = 0
				Crypto.Exchange.RebootInfo.state = false
				TriggerServerEvent('et-crypto:server:Rebooting', false, 0)
			end
			Wait(1200)
		end
	end)
				...
				if dist < 1.5 then
					if not Crypto.Exchange.RebootInfo.state then
						...
						if IsControlJustPressed(0, 38) then
							QBCore.Functions.TriggerCallback('et-crypto:server:HasSticky', function(HasItem)
								if HasItem then
...
RegisterNetEvent('et-crypto:client:SyncReboot', function()
	Crypto.Exchange.RebootInfo.state = true
	SystemCrashCooldown()
end)

RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
	TriggerServerEvent('et-crypto:server:FetchWorth')
	TriggerServerEvent('et-crypto:server:GetRebootState')
end)

RegisterNetEvent('et-crypto:client:UpdateCryptoWorth', function(crypto, amount, history)
...
RegisterNetEvent('et-crypto:client:GetRebootState', function(RebootInfo)

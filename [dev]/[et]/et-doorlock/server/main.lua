...
QBCore.Functions.CreateCallback('et-doorlock:server:setupDoors', function(_, cb)
	cb(Config.DoorList)
end)

QBCore.Functions.CreateCallback('et-doorlock:server:checkItems', function(source, cb, items, needsAll)
...
RegisterNetEvent('et-doorlock:server:updateState', function(doorID, locked, src, usedLockpick, unlockAnyway, enableSounds, enableAnimation, sentSource)
	...
	TriggerClientEvent('et-doorlock:client:setState', -1, playerId, doorID, locked, src or false, enableSounds, enableAnimation)
	...
	SetTimeout(Config.DoorList[doorID].autoLock, function()
		...
		if Config.DoorStates[doorID] == nil then Config.DoorStates[doorID] = locked elseif Config.DoorStates[doorID] ~= locked then Config.DoorStates[doorID] = nil end
		TriggerClientEvent('et-doorlock:client:setState', -1, playerId, doorID, true, src or false, enableSounds, enableAnimation)
	end)
end)

RegisterNetEvent('et-doorlock:server:saveNewDoor', function(data, doubleDoor)
	local src = source
	...
	TriggerClientEvent('et-doorlock:client:newDoorAdded', -1, configData, identifier, src)
end)
...
RegisterNetEvent('et-doorlock:server:removeLockpick', function(type)
	local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Commands.Add('newdoor', Lang:t("general.newdoor_command_description"), {}, false, function(source)
	TriggerClientEvent('et-doorlock:client:addNewDoor', source)
...
QBCore.Commands.Add('doordebug', Lang:t("general.doordebug_command_description"), {}, false, function(source)
	TriggerClientEvent('et-doorlock:client:ToggleDoorDebug', source)
end, Config.CommandPermission)

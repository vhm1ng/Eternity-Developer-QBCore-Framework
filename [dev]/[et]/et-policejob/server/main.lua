            ...
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    if billed.Functions.RemoveMoney('bank', amount, "paid-fine") then
                        ...
                        exports['et-management']:AddMoney(biller.PlayerData.job.name, amount)
                    elseif billed.Functions.RemoveMoney('cash', amount, "paid-fine") then
                        ...
                        MySQL.Async.insert('INSERT INTO phone_invoices (citizenid, amount, society, sender, sendercitizenid) VALUES (?, ?, ?, ?, ?)',{billed.PlayerData.citizenid, amount, biller.PlayerData.job.name, biller.PlayerData.charinfo.firstname, biller.PlayerData.citizenid}, function(id)
                            if id then
                                TriggerClientEvent('et-phone:client:AcceptorDenyInvoice', billed.PlayerData.source, id, biller.PlayerData.charinfo.firstname, biller.PlayerData.job.name, biller.PlayerData.citizenid, amount, GetInvokingResource())
                            end
                        end)
...
RegisterNetEvent('police:server:SendTrackerLocation', function(coords, requestId)
    ...
    TriggerClientEvent("et-phone:client:addPoliceAlert", requestId, alertData)
end)

QBCore.Commands.Add('113', Lang:t("commands.police_report"), {{name='message', help= Lang:t("commands.message_sent")}}, false, function(source, args)
	local src = source
    local message
    ...
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == 'police' and v.PlayerData.job.onduty then
            ...
            TriggerClientEvent("et-phone:client:addPoliceAlert", v.PlayerData.source, alertData)
...
RegisterNetEvent('police:server:BillPlayer', function(playerId, price)
    ...
    exports['et-management']:AddMoney("police", price)

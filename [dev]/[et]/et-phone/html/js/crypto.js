...
function LoadCryptoCoins(){
    $.post('https://et-phone/GetCryptosFromDegens', JSON.stringify({}), function(Jobs){
        $(".crypto-lists").html("");
...
$(document).on('click', '#crypto-send-purchase', function(e){
    ...
    if(amount != ""){
        ...
        $.post('https://et-phone/BuyCrypto', JSON.stringify({
            metadata: crypto,
            amount: amount,
        }));
    }
...
$(document).on('click', '#crypto-send-exchange', function(e){
    ...
    if(amount != "" || stateid != ""){
        if (CryptoType[crypto] - amount >= 0){
            ...
            $.post('https://et-phone/ExchangeCrypto', JSON.stringify({
                metadata: crypto,
                amount: amount,
                stateid: stateid,
            }));
...
$(document).on('click', '#crypto-send-sell', function(e){
    ...
    if(amount != ""){
        if (CryptoType[crypto] - amount >= 0){
            ...
            $.post('https://et-phone/SellCrypto', JSON.stringify({
                metadata: crypto,
                amount: amount,
            }));
        }else{

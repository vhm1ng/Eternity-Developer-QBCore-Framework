        ...
        if (CanOpenApp) {
            if (QB.Phone.Data.currentApplication == null) {
                ...
                if (PressedApplication == "twitter") {
                    ...
                    $.post('https://et-phone/GetJobs', JSON.stringify({}), function(data){
                        LoadEmploymentApp(data)
                    });
                } else if (PressedApplication == "debt") {
                    $.post('https://et-phone/GetPlayersDebt', JSON.stringify({}), function(data){
                        LoadDebtJob(data);
                    });
                }
                else if (PressedApplication == "gopro") {
                    $.post('https://et-phone/SetupGoPros', JSON.stringify({}), function(Cams){
                        SetupGoPros(Cams);
                    })
                }
                    ...
                    $.post('https://et-phone/GetChatRooms', JSON.stringify({}), function(ChatRooms){
...
$(document).on('click', '.phone-silent-button', function(event){
    event.preventDefault();
    $.post('https://et-phone/phone-silent-button', JSON.stringify({}),function(Data){
        if(Data){
...
$(document).on('click', ".notification-accept", function() {
    $.post('https://et-phone/AcceptNotification', JSON.stringify({})),
    QB.Phone.Animations.TopSlideUp(".phone-notification-container-new", 150, -8);
    QB.Phone.Notifications.Timeout = null
...
$(document).on('click', ".notification-deny", function() {
    $.post('https://et-phone/DenyNotification', JSON.stringify({})),
    QB.Phone.Notifications.Timeout = null

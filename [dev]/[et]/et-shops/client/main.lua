...
local function openShop(shop, data)
    QBCore.Functions.TriggerCallback('et-shops:server:SetShopInv', function(shopInvJson)
        ...
        while listen do
            ...
            if IsControlJustPressed(0, 38) then -- E
                if inChips then
                    ...
                    TriggerServerEvent("et-shops:server:sellChips")
                else
                    exports["et-core"]:KeyPressed()
...
local function createPeds()
    ...
    for k, v in pairs(Config.Locations) do
        ...
        if Config.UseTarget then
            exports['et-target']:AddTargetEntity(ShopPed[k], {
                options = {
                    {
                        label = v["targetLabel"],
                        icon = v["targetIcon"],
    ...
    if Config.UseTarget then
        exports['et-target']:AddTargetEntity(ShopPed["casino"], {
            options = {
                {
                    label = 'Sell Chips',
...
-- Events
RegisterNetEvent("et-shops:client:UpdateShop", function(shop, itemData, amount)
    TriggerServerEvent("et-shops:server:UpdateShopItems", shop, itemData, amount)
end)

RegisterNetEvent("et-shops:client:SetShopItems", function(shop, shopProducts)
    Config.Locations[shop]["products"] = shopProducts
end)

RegisterNetEvent("et-shops:client:RestockShopItems", function(shop, amount)
    ...
    CreateThread(function()
        ...
        combo:onPlayerInOut(function(isPointInside, _, zone)
            if isPointInside then
                ...
                exports["et-core"]:DrawText(Lang:t("info.open_shop"))
                listenForControl()
            else
        ...
        sellChips:onPlayerInOut(function(isPointInside)
            if isPointInside then
                inChips = true
                exports["et-core"]:DrawText(Lang:t("info.sell_chips"))
            else
                inChips = false
                exports["et-core"]:HideText()
            end

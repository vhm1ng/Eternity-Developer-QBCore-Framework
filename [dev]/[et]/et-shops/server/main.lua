...
--Events
QBCore.Functions.CreateCallback('et-shops:server:SetShopInv', function(_,cb)
    local shopInvJson = LoadResourceFile(GetCurrentResourceName(), Config.ShopsInvJsonFile)
    cb(shopInvJson)
end)
RegisterNetEvent('et-shops:server:SaveShopInv',function()
...
RegisterNetEvent('et-shops:server:UpdateShopItems', function(shop, itemData, amount)
    if not Config.UseTruckerJob then return end
    ...
    TriggerEvent('et-shops:server:SaveShopInv')
    TriggerClientEvent('et-shops:client:SetShopItems', -1, shop, Config.Locations[shop].products)
end)
RegisterNetEvent('et-shops:server:RestockShopItems', function(shop)
    ...
    TriggerEvent('et-shops:server:SaveShopInv')
    TriggerClientEvent('et-shops:client:RestockShopItems', -1, shop, randAmount)
end)
local ItemList = {
    ["casinochips"] = 1,
}
RegisterNetEvent('et-shops:server:sellChips', function()
    local src = source
        ...
        for k in pairs(Player.PlayerData.items) do
            if Player.PlayerData.items[k] then
                if ItemList[Player.PlayerData.items[k].name] then
                    ...
                    TriggerEvent("et-log:server:CreateLog", "casino", "Chips", "blue", "**" .. GetPlayerName(src) .. "** got $" .. price .. " for selling the Chips")
                end
            end
        end
    else
...
RegisterNetEvent('et-shops:server:SetShopList',function()
    local shoplist = {}
    local cnt = 0
    for k, v in pairs(Config.Locations) do
        cnt = cnt + 1
    ...
    TriggerClientEvent('et-truckerjob:client:SetShopList',-1,shoplist)
end)

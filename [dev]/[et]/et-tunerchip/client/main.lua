...
RegisterNUICallback('save', function(data, cb)
    QBCore.Functions.TriggerCallback('et-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            ...
            TriggerServerEvent('et-tunerchip:server:TuneStatus', QBCore.Functions.GetPlate(veh), true)
        end
        cb('ok')
    end)
end)

RegisterNetEvent('et-tunerchip:client:TuneStatus', function()
    local ped = PlayerPedId()
    ...
    if vehModel ~= 0 then
        QBCore.Functions.TriggerCallback('et-tunerchip:server:GetStatus', function(status)
            if status then
...
RegisterNetEvent('et-tunerchip:client:openChip', function()
    local ped = PlayerPedId()

...
RegisterNetEvent('hospital:server:SendToBed', function(bedId, isRevive)
	...
	Player.Functions.RemoveMoney("bank", Config.BillCost , "respawned-at-hospital")
		exports['et-management']:AddMoney("ambulance", Config.BillCost)
...
RegisterNetEvent('hospital:server:RevivePlayer', function(playerId, isOldMan)
	...
	if Patient then
		if Player.PlayerData.job.name == "ambulance" or QBCore.Functions.HasItem(src, "firstaid", 1) then
			...
			TriggerEvent('et-log:server:CreateLog', 'ambulancejob', 'Player Banned', 'red', string.format('%s was banned by %s for %s', GetPlayerName(src), 'et-ambulancejob', "Trying to revive theirselves or other players"), true)

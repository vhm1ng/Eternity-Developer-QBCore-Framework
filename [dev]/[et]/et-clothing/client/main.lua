...
local function SaveSkin()
    ...
    TriggerServerEvent("et-clothing:saveSkin", model, clothing)
end
...
local function reloadSkin(health)
    ...
    TriggerServerEvent("et-clothes:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES
    TriggerServerEvent("et-clothing:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES - Event 2
...
local function getOutfits(gradeLevel, data)
    ...
    QBCore.Functions.TriggerCallback('et-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNetEvent('et-clothing:client:openMenu')
AddEventHandler('et-clothing:client:openMenu', function()
...
RegisterNetEvent('et-clothing:client:reloadOutfits')
AddEventHandler('et-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
        action = "reloadMyOutfits",
        outfits = myOutfits
    })
end)
RegisterNetEvent('et-clothes:client:CreateFirstCharacter')
AddEventHandler('et-clothes:client:CreateFirstCharacter', function()
...
RegisterNetEvent("et-clothes:loadSkin")
AddEventHandler("et-clothes:loadSkin", function(_, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('et-clothing:client:loadPlayerClothing', data, PlayerPedId())
    end)
end)
RegisterNetEvent('et-clothing:client:loadPlayerClothing')
AddEventHandler('et-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('et-clothing:client:loadOutfit')
AddEventHandler('et-clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
...
RegisterNetEvent("et-clothing:client:adjustfacewear")
AddEventHandler("et-clothing:client:adjustfacewear",function(type)
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("et-clothes:loadPlayerSkin")
    PlayerData = QBCore.Functions.GetPlayerData()
--    QBCore.Shared.Jobs = exports['et-jobs']:AddJobs()
end)
...
RegisterNetEvent('et-clothing:client:openOutfitMenu', function()
    QBCore.Functions.TriggerCallback('et-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data, cb)
    TriggerEvent('et-clothing:client:loadOutfit', data)
    cb('ok')
end)
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('et-clothes:saveOutfit', data.outfitName, model, skinData)
    cb('ok')
end)
...
RegisterNUICallback('close', function(_, cb)
    ...
    disableCam()
    FreezeEntityPosition(PlayerPedId(), false)
    TriggerEvent('et-clothing:client:onMenuClose')
    cb('ok')
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('et-clothing:server:removeOutfit', data.outfitName, data.outfitId)
...
if Config.UseTarget then
    CreateThread(function()
        for k, v in pairs(Config.Stores) do
            ...
            exports['et-target']:AddBoxZone(v.shopType .. k, v.coords, v.length, v.width, {
                name = v.shopType .. k,
                debugPoly = false,
                minZ = v.coords.z-1,
                maxZ = v.coords.z+1,
        ...
        for k, v in pairs(Config.ClothingRooms) do
            ...
            exports['et-target']:AddBoxZone('clothing_' .. v.requiredJob .. k, v.coords, v.length, v.width, {
        ...
        clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
            if isPointInside then
                ...
                if zoneName == 'surgeon' then
                    exports['et-core']:DrawText('[E] - Plastic Surgery', 'left')
                elseif zoneName == 'clothing' then
                    exports['et-core']:DrawText('[E] - Clothing Shop', 'left')
                elseif zoneName == 'barber' then
                    exports['et-core']:DrawText('[E] - Barber', 'left')
                end
            else
                inZone = false

...
RegisterServerEvent("et-clothing:saveSkin", function(model, skin)
    local src = source
...
RegisterServerEvent("et-clothes:loadPlayerSkin", function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local result = MySQL.query.await('SELECT * FROM playerskins WHERE citizenid = ? AND active = ?', { Player.PlayerData.citizenid, 1 })
    if result[1] ~= nil then
        TriggerClientEvent("et-clothes:loadSkin", src, false, result[1].model, result[1].skin)
    else
        TriggerClientEvent("et-clothes:loadSkin", src, true)
    end
end)

RegisterServerEvent("et-clothes:saveOutfit", function(outfitName, model, skinData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if model ~= nil and skinData ~= nil then
        ...
        MySQL.insert('INSERT INTO player_outfits (citizenid, outfitname, model, skin, outfitId) VALUES (?, ?, ?, ?, ?)', {
            ...
            if result[1] ~= nil then
                TriggerClientEvent('et-clothing:client:reloadOutfits', src, result)
            else
                TriggerClientEvent('et-clothing:client:reloadOutfits', src, nil)
            end
        end)
    end
end)

RegisterServerEvent("et-clothing:server:removeOutfit", function(outfitName, outfitId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('et-clothing:server:getOutfits', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local anusVal = {}

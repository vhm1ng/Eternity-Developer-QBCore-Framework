    ...
    QBCore.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            QBCore.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                local data = exports['et-interior']:CreateApartmentFurnished(coords)
                Wait(100)
                HouseObj = data[1]
                POIOffsets = data[2]
                InApartment = true
                ...
                Wait(100)
                TriggerServerEvent('et-apartments:server:SetInsideMeta', house, apartmentId, true, false)
            ...
            TriggerEvent('et-weathersync:client:DisableSync')
            Wait(100)
            TriggerServerEvent('et-apartments:server:SetInsideMeta', house, apartmentId, true, true)
            TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_close", 0.1)
            TriggerServerEvent("apartments:server:setCurrentApartment", CurrentApartment)
        end
        ...
        if new ~= nil then
            if new then
                TriggerEvent('et-interior:client:SetNewState', true)
            else
                TriggerEvent('et-interior:client:SetNewState', false)
            end
        else
            TriggerEvent('et-interior:client:SetNewState', false)
        end
...
local function LeaveApartment(house)
    TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_open", 0.1)
    openHouseAnim()
    TriggerServerEvent("et-apartments:returnBucket")
    DoScreenFadeOut(500)
    ...
    exports['et-interior']:DespawnInterior(HouseObj, function()
        ...
        TriggerServerEvent('et-apartments:server:SetInsideMeta', CurrentApartment, false)
...
function MenuOwners()
    QBCore.Functions.TriggerCallback('apartments:GetAvailableApartments', function(apartments)
        if next(apartments) == nil then
            ...
            exports['et-menu']:openMenu(apartmentMenu)
        end
    end, ClosestHouse)
end
...
RegisterNetEvent('apartments:client:setupSpawnUI', function(cData)
    QBCore.Functions.TriggerCallback('apartments:GetOwnedApartment', function(result)
        if result then
            TriggerEvent('et-spawn:client:setupSpawns', cData, false, nil)
            TriggerEvent('et-spawn:client:openUI', true)
            TriggerEvent("apartments:client:SetHomeBlip", result.type)
        else
            if Apartments.Starting then
                TriggerEvent('et-spawn:client:setupSpawns', cData, true, Apartments.Locations)
                TriggerEvent('et-spawn:client:openUI', true)
            else
                TriggerEvent('et-spawn:client:setupSpawns', cData, false, nil)
                TriggerEvent('et-spawn:client:openUI', true)
...
RegisterNetEvent('et-apartments:client:LastLocationHouse', function(apartmentType, apartmentId)

...
QBCore.Functions.CreateCallback('et-vehicletuning:server:GetDrivingDistances', function(_, cb)
    cb(VehicleDrivingDistance)
end)

QBCore.Functions.CreateCallback('et-vehicletuning:server:IsVehicleOwned', function(_, cb, plate)
    local retval = false
    local result = MySQL.scalar.await('SELECT 1 from player_vehicles WHERE plate = ?', {plate})
    if result then
        retval = true
...
QBCore.Functions.CreateCallback('et-vehicletuning:server:GetAttachedVehicle', function(_, cb)
    cb(Config.Plates)
end)

QBCore.Functions.CreateCallback('et-vehicletuning:server:IsMechanicAvailable', function(_, cb)
    local amount = 0
...
QBCore.Functions.CreateCallback('et-vehicletuning:server:GetStatus', function(_, cb, plate)
...
RegisterNetEvent('et-vehicletuning:server:SaveVehicleProps', function(vehicleProps)
...
RegisterNetEvent('et-vehicletuning:server:UpdateDrivingDistance', function(amount, plate)
    VehicleDrivingDistance[plate] = amount
    TriggerClientEvent('et-vehicletuning:client:UpdateDrivingDistance', -1, VehicleDrivingDistance[plate], plate)
...
RegisterNetEvent('et-vehicletuning:server:LoadStatus', function(veh, plate)
...
RegisterNetEvent('et-vehicletuning:server:SetPartLevel', function(plate, part, level)
    if VehicleStatus[plate] ~= nil then
        VehicleStatus[plate][part] = level
        TriggerClientEvent("vehiclemod:client:setVehicleStatus", -1, plate, VehicleStatus[plate])
    end
...
RegisterNetEvent('et-vehicletuning:server:SetAttachedVehicle', function(veh, k)
    if veh ~= false then
        Config.Plates[k].AttachedVehicle = veh
        TriggerClientEvent('et-vehicletuning:client:SetAttachedVehicle', -1, veh, k)
    else
        Config.Plates[k].AttachedVehicle = nil
        TriggerClientEvent('et-vehicletuning:client:SetAttachedVehicle', -1, false, k)
    end
end)

RegisterNetEvent('et-vehicletuning:server:CheckForItems', function(part)
    local src = source
    ...
    if RepairPart ~= nil then
        if RepairPart.amount >= Config.RepairCostAmount[part].costs then
            TriggerClientEvent('et-vehicletuning:client:RepaireeePart', src, part)
...
RegisterNetEvent('et-mechanicjob:server:removePart', function(part, amount)

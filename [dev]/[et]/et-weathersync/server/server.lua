...
local function nextWeatherStage()
    ...
    TriggerEvent("et-weathersync:server:RequestStateSync")
end
...
-- EVENTS
RegisterNetEvent('et-weathersync:server:RequestStateSync', function()
    TriggerClientEvent('et-weathersync:client:SyncWeather', -1, CurrentWeather, blackout)
    TriggerClientEvent('et-weathersync:client:SyncTime', -1, baseTime, timeOffset, freezeTime)
end)

RegisterNetEvent('et-weathersync:server:setWeather', function(weather)
...
RegisterNetEvent('et-weathersync:server:setTime', function(hour, minute)
    local src = getSource(source)
    if isAllowedToChange(src) then
...
RegisterNetEvent('et-weathersync:server:toggleBlackout', function(state)
    local src = getSource(source)
    if isAllowedToChange(src) then
...
RegisterNetEvent('et-weathersync:server:toggleFreezeTime', function(state)
    local src = getSource(source)
    if isAllowedToChange(src) then
...
RegisterNetEvent('et-weathersync:server:toggleDynamicWeather', function(state)
    local src = getSource(source)
    if isAllowedToChange(src) then

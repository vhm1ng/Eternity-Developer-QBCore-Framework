...
local function DestroyPickupTarget()
  ...
  if Config.UseTarget then
    exports['et-target']:RemoveZone(pickupTargetID)
    pickupZone = nil
  else
    pickupZone:destroy()
    pickupZone = nil
...
local function RegisterEntranceTarget()
  ...
  if Config.UseTarget then
    entranceZone = exports['et-target']:AddBoxZone(entranceTargetID, coords, 1, 4, {
      ...
      options = {
        {
          type = 'client',
          event = 'et-recyclejob:client:target:enterLocation',
    ...
    entranceZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['et-core']:DrawText(Lang:t("text.point_enter_warehouse"), 'left')
      else
...
local function RegisterExitTarget()
  ...
  if Config.UseTarget then
    exitZone = exports['et-target']:AddBoxZone(exitTargetID, coords, 1, 4, {
      name = exitTargetID,
      heading = 270,
      ...
      options = {
        {
          type = 'client',
          event = 'et-recyclejob:client:target:exitLocation',
    ...
    exitZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['et-core']:DrawText(Lang:t("text.point_exit_warehouse"), 'left')
      else
        exports['et-core']:HideText()
      end
...
local function DestroyExitTarget()
  ...
  if Config.UseTarget then
    exports['et-target']:RemoveZone(exitTargetID)
    exitZone = nil
  else
    exitZone:destroy()
    exitZone = nil
...
local function RegisterDutyTarget()
  ...
  if Config.UseTarget then
    dutyZone = exports['et-target']:AddBoxZone(dutyTargetID, coords, 1, 1, {
      name = dutyTargetID,
      heading = 270,
      ...
      options = {
        {
          type = 'client',
          event = 'et-recyclejob:client:target:toggleDuty',
    ...
    dutyZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['et-core']:DrawText(GetDutyTargetText(), 'left')
      else
        exports['et-core']:HideText()
      end
...
local function DestroyDutyTarget()
  ...
  if Config.UseTarget then
    exports['et-target']:RemoveZone(dutyTargetID)
    dutyZone = nil
  else
    dutyZone:destroy()
    dutyZone = nil
...
local function RegisterDeliveyTarget()
  ...
  if Config.UseTarget then
    deliveryZone = exports['et-target']:AddBoxZone(deliveryTargetID, coords, 1, 1, {
      ...
      options = {
        {
          type = 'client',
          event = 'et-recyclejob:client:target:dropPackage',
    ...
    deliveryZone:onPlayerInOut(function(isPointInside)
      if isPointInside and carryPackage then
        exports['et-core']:DrawText(Lang:t("text.point_hand_in_package"), 'left')
      else
        exports['et-core']:HideText()
      end
...
local function DestroyDeliveryTarget()
  ...
  if Config.UseTarget then
    exports['et-target']:RemoveZone(deliveryTargetID)
    deliveryZone = nil
  else
...
local function SetLocationBlip()
  ...
  AddTextComponentString('~b~[Nghề Nghiệp] Tái chế')
...
function RegisterPickupTarget(coords)
  ...
  if Config.UseTarget then
    pickupZone = exports['et-target']:AddBoxZone(pickupTargetID, targetCoords, 4, 1.5, {
      name = pickupTargetID,
      heading = coords.h,
      minZ = coords.z - 1.0,
      maxZ = coords.z + 2.0,
      ...
      options = {
        {
          type = 'client',
          event = 'et-recyclejob:client:target:pickupPackage',
    ...
    pickupZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['et-core']:DrawText( Lang:t("text.point_get_package"), 'left')
      else
        exports['et-core']:HideText()
      end
...
RegisterNetEvent('et-recyclejob:client:target:enterLocation', function()
  EnterLocation()
end)

RegisterNetEvent('et-recyclejob:client:target:exitLocation', function()
  ExitLocation()
end)

RegisterNetEvent('et-recyclejob:client:target:toggleDuty', function()
  onDuty = not onDuty
  if onDuty then
...
RegisterNetEvent('et-recyclejob:client:target:pickupPackage', function()
...
RegisterNetEvent('et-recyclejob:client:target:dropPackage', function()
  ...
  QBCore.Functions.Progressbar('deliver_reycle_package',  Lang:t("text.unpacking_the_package"), Config.DeliveryActionDuration, false, true, {
      ...
      -- Done
      StopAnimTask(PlayerPedId(), 'mp_car_bomb', 'car_bomb_mechanic', 1.0)
      TriggerServerEvent('et-recycle:server:getItem')
      GetRandomPackage()
  end)
end)
    ...
    while true do
      ...
      if isInsideEntranceZone then
        ...
        if IsControlJustReleased(0, 38) then
          ...
          Wait(500)
          TriggerEvent('et-recyclejob:client:target:enterLocation')
          exports['et-core']:HideText()
        end
      end
      ...
      if isInsideExitZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['et-core']:KeyPressed()
          Wait(500)
          TriggerEvent('et-recyclejob:client:target:exitLocation')
          exports['et-core']:HideText()
        end
      end
      ...
      if isInsideDutyZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['et-core']:KeyPressed()
          Wait(500)
          TriggerEvent('et-recyclejob:client:target:toggleDuty')
          exports['et-core']:HideText()
        end
      end
      ...
      if onDuty then
        if isInsidePickupZone and not carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['et-core']:KeyPressed()
            Wait(500)
            TriggerEvent('et-recyclejob:client:target:pickupPackage')
            exports['et-core']:HideText()
          end
        ...
        if isInsideDeliveryZone and carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['et-core']:KeyPressed()
            Wait(500)
            TriggerEvent('et-recyclejob:client:target:dropPackage')
            exports['et-core']:HideText()
          end
        end
      end

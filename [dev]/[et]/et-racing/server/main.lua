...
---- Server Events ----
-----------------------
RegisterNetEvent('et-racing:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    local src = source
    ...
    TriggerClientEvent('et-racing:client:PlayerFinish', -1, RaceData.RaceId, PlayersFinished, RacerName)
...
RegisterNetEvent('et-racing:server:CreateLapRace', function(RaceName, RacerName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)

    if IsPermissioned(Player.PlayerData.citizenid, 'create') then
        if IsNameAvailable(RaceName) then
            TriggerClientEvent('et-racing:client:StartRaceEditor', source, RaceName, RacerName)
        else
...
RegisterNetEvent('et-racing:server:JoinRace', function(RaceData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if CurrentRace ~= nil then
        ...
        if (AmountOfRacers - 1) == 0 then
            ...
            TriggerClientEvent('et-racing:client:LeaveRace', src, Races[CurrentRace])
        else
    ...
    TriggerClientEvent('et-racing:client:JoinRace', src, Races[RaceId], RaceData.Laps, RacerName)
    TriggerClientEvent('et-racing:client:UpdateRaceRacers', src, RaceId, Races[RaceId].Racers)
...
RegisterNetEvent('et-racing:server:LeaveRace', function(RaceData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local RacerName = RaceData.RacerName
    ...
    if (AmountOfRacers - 1) == 0 then
        ...
        table.remove(AvailableRaces, AvailableKey)
        TriggerClientEvent('QBCore:Notify', src, Lang:t("primary.race_last_person"))
        TriggerClientEvent('et-racing:client:LeaveRace', src, Races[RaceId])
...
RegisterNetEvent('et-racing:server:SetupRace', function(RaceId, Laps, RacerName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Races[RaceId] ~= nil then
        if not Races[RaceId].Waiting then
            if not Races[RaceId].Started then
                ...
                TriggerClientEvent('et-racing:server:ReadyJoinRace', src, allRaceData)
                        ...
                        if count < 5 * 60 then
                            ...
                            for cid, _ in pairs(Races[RaceId].Racers) do
                                ...
                                if RacerData ~= nil then
                                    ...
                                    TriggerClientEvent('et-racing:client:LeaveRace', RacerData.PlayerData.source, Races[RaceId])
                                end
                            end
                            table.remove(AvailableRaces, AvailableKey)
...
RegisterNetEvent('et-racing:server:UpdateRaceState', function(RaceId, Started, Waiting)
    Races[RaceId].Waiting = Waiting
    Races[RaceId].Started = Started
end)

RegisterNetEvent('et-racing:server:UpdateRacerData', function(RaceId, Checkpoint, Lap, Finished)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('et-racing:client:UpdateRaceRacerData', -1, RaceId, Races[RaceId])
end)

RegisterNetEvent('et-racing:server:StartRace', function(RaceId)
    local src = source
    ...
    for CitizenId, _ in pairs(Races[RaceId].Racers) do
        ...
        if Player ~= nil then
            TriggerClientEvent('et-racing:client:RaceCountdown', Player.PlayerData.source)
        end
    end
end)

RegisterNetEvent('et-racing:server:SaveRace', function(RaceData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
function UseRacingFob(source, item)
    ...
    if item.info.owner == citizenid then
        TriggerClientEvent('et-racing:Client:OpenMainMenu', source, { type = item.name, name = item.info.name})
    else
        TriggerClientEvent('QBCore:Notify', source, Lang:t("error.unowned_dongle"), "error")
    end
end

QBCore.Functions.CreateCallback('et-racing:server:GetRacingLeaderboards', function(source, cb)
    local Leaderboard = {}
...
QBCore.Functions.CreateCallback('et-racing:server:GetRaces', function(source, cb)
    cb(AvailableRaces)
end)

QBCore.Functions.CreateCallback('et-racing:server:GetListedRaces', function(source, cb)
    cb(Races)
end)

QBCore.Functions.CreateCallback('et-racing:server:GetRacingData', function(source, cb, RaceId)
    cb(Races[RaceId])
end)

QBCore.Functions.CreateCallback('et-racing:server:HasCreatedRace', function(source, cb)
    cb(HasOpenedRace(QBCore.Functions.GetPlayer(source).PlayerData.citizenid))
end)

QBCore.Functions.CreateCallback('et-racing:server:IsAuthorizedToCreateRaces', function(source, cb, TrackName)
    cb(IsPermissioned(QBCore.Functions.GetPlayer(source).PlayerData.citizenid, 'create'), IsNameAvailable(TrackName))
end)

QBCore.Functions.CreateCallback('et-racing:server:GetTrackData', function(source, cb, RaceId)

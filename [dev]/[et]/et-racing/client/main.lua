...
function SaveRace()
    ...
    TriggerServerEvent('et-racing:server:SaveRace', CreatorData)
...
function FinishRace()
    TriggerServerEvent('et-racing:server:FinishPlayer', CurrentRaceData, CurrentRaceData.TotalTime, CurrentRaceData.TotalLaps, CurrentRaceData.BestLap)
                ...
                if CheckpointDistance < MaxDistance then
                    if CurrentRaceData.TotalLaps == 0 then
                        if CurrentRaceData.CurrentCheckpoint + 1 < #CurrentRaceData.Checkpoints then
                            ...
                            TriggerServerEvent('et-racing:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, false)
                            DoPilePfx()
                            ...
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            CurrentRaceData.CurrentCheckpoint = CurrentRaceData.CurrentCheckpoint + 1
                            TriggerServerEvent('et-racing:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, true)
                            FinishRace()
                        end
                    else
                        if CurrentRaceData.CurrentCheckpoint + 1 > #CurrentRaceData.Checkpoints then
...
RegisterNetEvent('et-racing:server:ReadyJoinRace', function(RaceData)
    RaceData.RacerName = RaceData.SetupRacerName
    TriggerServerEvent('et-racing:server:JoinRace', RaceData)
end)

RegisterNetEvent('et-racing:client:StartRaceEditor', function(RaceName, RacerName)
...
RegisterNetEvent('et-racing:client:UpdateRaceRacerData', function(RaceId, RaceData)
    if (CurrentRaceData.RaceId ~= nil) and CurrentRaceData.RaceId == RaceId then
        CurrentRaceData.Racers = RaceData.Racers
    end
end)

RegisterNetEvent('et-racing:client:JoinRace', function(Data, Laps, RacerName)
    if not RaceData.InRace then
        ...
        TriggerServerEvent('et-racing:server:UpdateRaceState', CurrentRaceData.RaceId, false, true)
    else
        QBCore.Functions.Notify(Lang:t("error.already_in_race"), 'error')
    end
end)

RegisterNetEvent('et-racing:client:UpdateRaceRacers', function(RaceId, Racers)
    if CurrentRaceData.RaceId == RaceId then
        CurrentRaceData.Racers = Racers
    end
end)

RegisterNetEvent('et-racing:client:LeaveRace', function(data)
    DeleteCurrentRaceCheckpoints()
    FreezeEntityPosition(GetVehiclePedIsIn(PlayerPedId(), false), false)
end)

RegisterNetEvent('et-racing:client:RaceCountdown', function()
    TriggerServerEvent('et-racing:server:UpdateRaceState', CurrentRaceData.RaceId, true, false)
...
RegisterNetEvent('et-racing:client:PlayerFinish', function(RaceId, Place, RacerName)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
...
RegisterNetEvent('et-racing:client:WaitingDistanceCheck', function()
    Wait(1000)
    CreateThread(function()
        while true do
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            TriggerServerEvent('et-racing:server:LeaveRace', CurrentRaceData)
                            ToFarCountdown = 10
                            break
                        end
                        Wait(1000)
...
RegisterNetEvent("et-racing:Client:OpenMainMenu", function(data)
    ...
    exports['et-menu']:openMenu({
        ...
        },
        {
            ...
            params = {
                event = "et-racing:Client:CurrentRaceMenu",
        ...
        },
        {
            ...
            params = {
                event = "et-racing:Client:AvailableRacesMenu",
                args = { type = type, name = name }
            }
        },
        {
            ...
            params = {
                event = "et-racing:Client:RaceRecordsMenu",
                args = { type = type, name = name }
            }
        },
        {
            ...
            params = {
                event = "et-racing:Client:SetupRaceMenu",
                args = { type = type, name = name }
            }
        },
        {
            ...
            params = {
                event = "et-racing:Client:CreateRaceMenu",
                args = { type = type, name = name }
            }
        },
        {
...
RegisterNetEvent("et-racing:Client:CurrentRaceMenu", function(data)
    ...
    exports['et-menu']:openMenu({
        ...
        },
        {
            ...
            params = {
                isServer = true,
                event = "et-racing:server:StartRace",
        ...
        },
        {
            ...
            params = {
                isServer = true,
                event = "et-racing:server:LeaveRace",
        ...
        },
        {
            ...
            params = {
                event = "et-racing:Client:OpenMainMenu",
...
RegisterNetEvent("et-racing:Client:AvailableRacesMenu", function(data)
    QBCore.Functions.TriggerCallback('et-racing:server:GetRaces', function(Races)
        local menu = {
            {
                header = Lang:t("menu.available_races"),
                isMenuHeader = true
        ...
        for _,race in ipairs(Races) do
            ...
            menu[#menu+1] = {
                ...
                params = {
                    isServer = true,
                    event = "et-racing:server:JoinRace",
                    args = race
                }
            }
        end
        ...
        if #menu == 2 then
            ...
            TriggerEvent('et-racing:Client:OpenMainMenu', { type = data.type, name = data.name })
            return
        end
...
RegisterNetEvent("et-racing:Client:RaceRecordsMenu", function(data)
    QBCore.Functions.TriggerCallback('et-racing:server:GetRacingLeaderboards', function(Races)
        local menu = {
            {
                header = Lang:t("menu.race_records"),
                isMenuHeader = true
...
RegisterNetEvent("et-racing:Client:SetupRaceMenu", function(data)
    QBCore.Functions.TriggerCallback('et-racing:server:GetListedRaces', function(Races)
        ...
        TriggerServerEvent('et-racing:server:SetupRace', dialog.track, tonumber(dialog.laps), data.name)
    end)
end)

RegisterNetEvent("et-racing:Client:CreateRaceMenu", function(data)
    local dialog = exports['et-input']:ShowInput({
    ...
    if #dialog.trackname < Config.MinTrackNameLength then
        ...
        TriggerEvent("et-racing:Client:CreateRaceMenu", { type = data.type, name = data.name })
        return
    end
    ...
    QBCore.Functions.TriggerCallback('et-racing:server:IsAuthorizedToCreateRaces', function(IsAuthorized, NameAvailable)
        ...
        TriggerServerEvent('et-racing:server:CreateLapRace', dialog.trackname, data.name)

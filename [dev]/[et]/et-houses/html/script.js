...
$('document').ready(function() {
    ...
    document.onkeyup = function (data) {
        if (data.which == 116 ) {
            $.post('https://et-houses/toggleCursor');
        }

        if (data.which == 13 ) {
            if (selectedObjectData != null && $(".decorate-confirm").css('display') != 'block') {
                $.post('https://et-houses/editOwnedObject', JSON.stringify({
                    objectData: selectedObjectData
                }));
                selectedObjectData = null;
                $.post('https://et-houses/setupMyObjects', JSON.stringify({}), function(myObjects){
                    $('.decorate-items').html("");
                ...
                if (objData != null) {
                    $.post("https://et-houses/spawnobject", JSON.stringify({
...
$(document).on('click', '.header-btn', function(){
    ...
    $.post('https://et-houses/ResetSelectedProp');
...
$(document).on('click', '.footer-btn', function(){
    ...
    if (selectedCategory != "remove-owned-obj") {
        ...
        $(".decorate-footer-buttons").html("");
        $(selectedHeaderButton).removeClass('header-btn-selected');
        $.post('https://et-houses/deleteSelectedObject');
        $(".decorate-footer-buttons").fadeOut(150);
        $(".decorate-items").fadeOut(150);
    }
});
...
$(document).on('click', '#buy-object', function(){
    $.post("https://et-houses/buySelectedObject", JSON.stringify({
...
$(document).on('click', '#cancel-object', function(){
    $.post('https://et-houses/cancelSelectedObject');
    selectedObjectData = null;
    $(".decorate-confirm").css("display", "none");
});
...
$(document).on('click', '.decorate-item', function(){
    ...
    if ($("#"+objId).data('type') == "newObject") {
        if (selectedObject == this) {
            ...
            selectedObject = null;
            $.post('https://et-houses/removeObject');
        } else {
            $(this).addClass('selected-object');
            selectedObject = this;
            $.post("https://et-houses/chooseobject", JSON.stringify({
                object: objData.object,
            }));
        ...
        if (selectedObject == this) {
            $(this).removeClass('selected-object');
            $.post('https://et-houses/deselectOwnedObject')
            selectedObject = null;
        } else {
            $(this).addClass('selected-object');
            selectedObject = this;
            selectedObjectData = myObjectData;
            $.post('https://et-houses/selectOwnedObject', JSON.stringify({
...
Decorations.Close = function() {
    ...
    selectedObjectData = null;
    $.post("https://et-houses/closedecorations", JSON.stringify({}));
}

$(".property-accept").click(function(e){
    $.post('https://et-houses/buy', JSON.stringify({}))
});

$(".property-cancel").click(function(e){
    $.post('https://et-houses/exit', JSON.stringify({}));
});

...
local function showEntranceHeaderMenu()
    ...
    if not isOwned then
        headerMenu[#headerMenu+1] = {
            ...
            params = {
                event = "et-houses:client:ViewHouse",
                args = {}
            }
        }
    else
        if isOwned and HasHouseKey then
            headerMenu[#headerMenu+1] = {
                ...
                params = {
                    event = "et-houses:client:EnterHouse",
                    args = {}
                }
            }
            headerMenu[#headerMenu+1] = {
        ...
        elseif isOwned and not HasHouseKey then
            headerMenu[#headerMenu+1] = {
                ...
                params = {
                    event = "et-houses:client:RequestRing",
                    args = {}
                }
            }
            headerMenu[#headerMenu+1] = {
    ...
    if headerMenu and next(headerMenu) then
        exports['et-menu']:openMenu(headerMenu)
    end
end
...
local function showExitHeaderMenu()
    ...
    headerMenu[#headerMenu+1] = {
        ...
        params = {
            event = "et-houses:client:ExitOwnedHouse",
            args = {}
        }
    }
    if isOwned then
        headerMenu[#headerMenu+1] = {
            ...
            params = {
                event = "et-houses:client:FrontDoorCam",
                args = {}
            }
        }
        headerMenu[#headerMenu+1] = {
            ...
            params = {
                event = "et-houses:client:AnswerDoorbell",
                args = {}
            }
        }
    end
...
local function RegisterStashTarget()
    ...
    stashTargetBox:onPlayerInOut(function (isPointInside)
        if isPointInside and not entering and isOwned then
            exports['et-core']:DrawText(Lang:t("target.open_stash"), 'left')
        else
...
local function RegisterOutfitsTarget()
    ...
    outfitsTargetBox:onPlayerInOut(function (isPointInside)
        if isPointInside and not entering and isOwned then
            exports['et-core']:DrawText(Lang:t("target.outfits"), 'left')
        else
            exports['et-core']:HideText()
        end
...
local function RegisterCharactersTarget()
    ...
    charactersTargetBox:onPlayerInOut(function (isPointInside)
        if isPointInside and not entering and isOwned then
            exports['et-core']:DrawText(Lang:t("target.change_character"), 'left')
        else
            exports['et-core']:HideText()
        end
...
RegisterNetEvent('et-houses:client:setHouseConfig', function(houseConfig)
...
local function FrontDoorCam(coords)
    ...
    SetCamActive(cam, true)
    RenderScriptCams(true, true, 500, true, true)
    TriggerEvent('et-weathersync:client:EnableSync')
    FrontCam = true
...
local function SetClosestHouse()
    ...
    if not IsInside then
        ...
        if ClosestHouse ~= nil and tonumber(dist) < 30 then
            QBCore.Functions.TriggerCallback('et-houses:server:ProximityKO', function(key, owned)
                HasHouseKey = key
                isOwned = owned
            end, ClosestHouse)
        end
    end
    TriggerEvent('et-garages:client:setHouseGarage', ClosestHouse, HasHouseKey)
end

local function setHouseLocations()
    if ClosestHouse ~= nil then
        QBCore.Functions.TriggerCallback('et-houses:server:getHouseLocations', function(result)
            if result ~= nil then
...
local function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		QBCore.Functions.TriggerCallback('et-houses:server:getHouseDecorations', function(result)
...
local function RemoveHouseKey(citizenData)
    TriggerServerEvent('et-houses:server:removeHouseKey', ClosestHouse, citizenData)
    CloseMenuFull()
end
...
local function getKeyHolders()
    ...
    local p = promise.new()
    QBCore.Functions.TriggerCallback('et-houses:server:getHouseKeyHolders', function(holders)
        p:resolve(holders)
    end,ClosestHouse)
    ...
    if holders == nil or next(holders) == nil then
        ...
        for k, _ in pairs(holders) do
            keyholderMenu[#keyholderMenu+1] = {
                ...
                params = {
                    event = "et-houses:client:OpenClientOptions",
                    args = {
        ...
        exports['et-menu']:openMenu(keyholderMenu)
    end
...
local function optionMenu(citizenData)
    keyholderOptions = {
        {
            ...
            params = {
                event = "et-houses:client:RevokeKey",
                args = {
    ...
    exports['et-menu']:openMenu(keyholderOptions)
end
...
local function getDataForHouseTier(house, coords)
    if Config.Houses[house].tier == 1 then
        return exports['et-interior']:CreateApartmentShell(coords)
    elseif Config.Houses[house].tier == 2 then
        return exports['et-interior']:CreateTier1House(coords)
    elseif Config.Houses[house].tier == 3 then
        return exports['et-interior']:CreateTrevorsShell(coords)
    elseif Config.Houses[house].tier == 4 then
        return exports['et-interior']:CreateCaravanShell(coords)
    elseif Config.Houses[house].tier == 5 then
        return exports['et-interior']:CreateLesterShell(coords)
    elseif Config.Houses[house].tier == 6 then
        return exports['et-interior']:CreateRanchShell(coords)
    elseif Config.Houses[house].tier == 7 then
        return exports['et-interior']:CreateContainer(coords)
    elseif Config.Houses[house].tier == 8 then
        return exports['et-interior']:CreateFurniMid(coords)
    elseif Config.Houses[house].tier == 9 then
        return exports['et-interior']:CreateFurniMotelModern(coords)
    elseif Config.Houses[house].tier == 10 then
        return exports['et-interior']:CreateFranklinAunt(coords)
    elseif Config.Houses[house].tier == 11 then
        return exports['et-interior']:CreateGarageMed(coords)
    elseif Config.Houses[house].tier == 12 then
        return exports['et-interior']:CreateMichael(coords)
    elseif Config.Houses[house].tier == 13 then
        return exports['et-interior']:CreateOffice1(coords)
    elseif Config.Houses[house].tier == 14 then
        return exports['et-interior']:CreateStore1(coords)
    elseif Config.Houses[house].tier == 15 then
        return exports['et-interior']:CreateWarehouse1(coords)
    elseif Config.Houses[house].tier == 16 then
        return exports['et-interior']:CreateFurniMotelStandard(coords) -- End of free shells
    elseif Config.Houses[house].tier == 17 then
        return exports['et-interior']:CreateMedium2(coords)
    elseif Config.Houses[house].tier == 18 then
        return exports['et-interior']:CreateMedium3(coords)
    elseif Config.Houses[house].tier == 19 then
        return exports['et-interior']:CreateBanham(coords)
    elseif Config.Houses[house].tier == 20 then
        return exports['et-interior']:CreateWestons(coords)
    elseif Config.Houses[house].tier == 21 then
        return exports['et-interior']:CreateWestons2(coords)
    elseif Config.Houses[house].tier == 22 then
        return exports['et-interior']:CreateClassicHouse(coords)
    elseif Config.Houses[house].tier == 23 then
        return exports['et-interior']:CreateClassicHouse2(coords)
    elseif Config.Houses[house].tier == 24 then
        return exports['et-interior']:CreateClassicHouse3(coords)
    elseif Config.Houses[house].tier == 25 then
        return exports['et-interior']:CreateHighend1(coords)
    elseif Config.Houses[house].tier == 26 then
        return exports['et-interior']:CreateHighend2(coords)
    elseif Config.Houses[house].tier == 27 then
        return exports['et-interior']:CreateHighend3(coords)
    elseif Config.Houses[house].tier == 28 then
        return exports['et-interior']:CreateHighend(coords)
    elseif Config.Houses[house].tier == 29 then
        return exports['et-interior']:CreateHighendV2(coords)
    elseif Config.Houses[house].tier == 30 then
        return exports['et-interior']:CreateStashHouse(coords)
    elseif Config.Houses[house].tier == 31 then
        return exports['et-interior']:CreateStashHouse2(coords)
    elseif Config.Houses[house].tier == 32 then
        return exports['et-interior']:CreateGarageLow(coords)
    elseif Config.Houses[house].tier == 33 then
        return exports['et-interior']:CreateGarageHigh(coords)
    elseif Config.Houses[house].tier == 34 then
        return exports['et-interior']:CreateOffice2(coords)
    elseif Config.Houses[house].tier == 35 then
        return exports['et-interior']:CreateOfficeBig(coords)
    elseif Config.Houses[house].tier == 36 then
        return exports['et-interior']:CreateBarber(coords)
    elseif Config.Houses[house].tier == 37 then
        return exports['et-interior']:CreateGunstore(coords)
    elseif Config.Houses[house].tier == 38 then
        return exports['et-interior']:CreateStore2(coords)
    elseif Config.Houses[house].tier == 39 then
        return exports['et-interior']:CreateStore3(coords)
    elseif Config.Houses[house].tier == 40 then
        return exports['et-interior']:CreateWarehouse2(coords)
    elseif Config.Houses[house].tier == 41 then
        return exports['et-interior']:CreateWarehouse3(coords)
    elseif Config.Houses[house].tier == 42 then
        return exports['et-interior']:CreateK4Coke(coords)
    elseif Config.Houses[house].tier == 43 then
        return exports['et-interior']:CreateK4Meth(coords)
    elseif Config.Houses[house].tier == 44 then
        return exports['et-interior']:CreateK4Weed(coords)
    elseif Config.Houses[house].tier == 45 then
        return exports['et-interior']:CreateContainer2(coords)
    elseif Config.Houses[house].tier == 46 then
        return exports['et-interior']:CreateFurniStash1(coords)
    elseif Config.Houses[house].tier == 47 then
        return exports['et-interior']:CreateFurniStash3(coords)
    elseif Config.Houses[house].tier == 48 then
        return exports['et-interior']:CreateFurniLow(coords)
    elseif Config.Houses[house].tier == 49 then
        return exports['et-interior']:CreateFurniMotel(coords)
    elseif Config.Houses[house].tier == 50 then
        return exports['et-interior']:CreateFurniMotelClassic(coords)
    elseif Config.Houses[house].tier == 51 then
        return exports['et-interior']:CreateFurniMotelHigh(coords)
    elseif Config.Houses[house].tier == 52 then
        return exports['et-interior']:CreateFurniMotelModern2(coords)
    elseif Config.Houses[house].tier == 53 then
        return exports['et-interior']:CreateFurniMotelModern3(coords)
    elseif Config.Houses[house].tier == 54 then
        return exports['et-interior']:CreateCoke(coords)
    elseif Config.Houses[house].tier == 55 then
        return exports['et-interior']:CreateCoke2(coords)
    elseif Config.Houses[house].tier == 56 then
        return exports['et-interior']:CreateMeth(coords)
    elseif Config.Houses[house].tier == 57 then
        return exports['et-interior']:CreateWeed(coords)
    elseif Config.Houses[house].tier == 58 then
        return exports['et-interior']:CreateWeed2(coords)
    elseif Config.Houses[house].tier == 59 then
        return exports['et-interior']:CreateMansion(coords)
    elseif Config.Houses[house].tier == 60 then
        return exports['et-interior']:CreateMansion2(coords)
    elseif Config.Houses[house].tier == 61 then
        return exports['et-interior']:CreateMansion3(coords)
    elseif Config.Houses[house].tier == 62 then
        return exports['et-interior']:CreateHotel1(coords)
    elseif Config.Houses[house].tier == 63 then
        return exports['et-interior']:CreateHotel2(coords)
    elseif Config.Houses[house].tier == 64 then
        return exports['et-interior']:CreateHotel3(coords)
    elseif Config.Houses[house].tier == 65 then
        return exports['et-interior']:CreateMotel1(coords)
    elseif Config.Houses[house].tier == 66 then
        return exports['et-interior']:CreateMotel2(coords)
    elseif Config.Houses[house].tier == 67 then
        return exports['et-interior']:CreateMotel3(coords)
    elseif Config.Houses[house].tier == 68 then
        return exports['et-interior']:CreateV2Default1(coords)
    elseif Config.Houses[house].tier == 69 then
        return exports['et-interior']:CreateV2Default2(coords)
    elseif Config.Houses[house].tier == 70 then
        return exports['et-interior']:CreateV2Default3(coords)
    elseif Config.Houses[house].tier == 71 then
        return exports['et-interior']:CreateV2Default4(coords)
    elseif Config.Houses[house].tier == 72 then
        return exports['et-interior']:CreateV2Default5(coords)
    elseif Config.Houses[house].tier == 73 then
        return exports['et-interior']:CreateV2Default6(coords)
    elseif Config.Houses[house].tier == 74 then
        return exports['et-interior']:CreateV2Deluxe1(coords)
    elseif Config.Houses[house].tier == 75 then
        return exports['et-interior']:CreateV2Deluxe2(coords)
    elseif Config.Houses[house].tier == 76 then
        return exports['et-interior']:CreateV2Deluxe3(coords)
    elseif Config.Houses[house].tier == 77 then
        return exports['et-interior']:CreateV2HighEnd1(coords)
    elseif Config.Houses[house].tier == 78 then
        return exports['et-interior']:CreateV2HighEnd2(coords)
    elseif Config.Houses[house].tier == 79 then
        return exports['et-interior']:CreateV2HighEnd3(coords)
    elseif Config.Houses[house].tier == 80 then
        return exports['et-interior']:CreateV2Medium1(coords)
    elseif Config.Houses[house].tier == 81 then
        return exports['et-interior']:CreateV2Medium2(coords)
    elseif Config.Houses[house].tier == 82 then
        return exports['et-interior']:CreateV2Medium3(coords)
    elseif Config.Houses[house].tier == 83 then
        return exports['et-interior']:CreateV2Modern1(coords)
    elseif Config.Houses[house].tier == 84 then
        return exports['et-interior']:CreateV2Modern2(coords)
    elseif Config.Houses[house].tier == 85 then
        return exports['et-interior']:CreateV2Modern3(coords)
    else
...
local function enterOwnedHouse(house)
    ...
    entering = true
    Wait(500)
    TriggerServerEvent('et-houses:server:SetInsideMeta', house, true)
    --TriggerEvent('et-weathersync:client:DisableSync')
    TriggerEvent('et-weathersync:client:EnableSync')
    TriggerEvent('et-weed:client:getHousePlants', house)
    entering = false
    setHouseLocations()
    CloseMenuFull()
...
local function LeaveHouse(house)
    if not FrontCam then
        ...
        DoScreenFadeOut(250)
        Wait(500)
        exports['et-interior']:DespawnInterior(houseObj, function()
            UnloadDecorations()
            TriggerEvent('et-weathersync:client:EnableSync')
            Wait(250)
            DoScreenFadeIn(250)
            ...
            TriggerEvent('et-weed:client:leaveHouse')
            TriggerServerEvent('et-houses:server:SetInsideMeta', house, false)
            CurrentHouse = nil
...
RegisterNetEvent('et-houses:server:sethousedecorations', function(house, decorations)
...
RegisterNetEvent('et-houses:client:sellHouse', function()
    if ClosestHouse and HasHouseKey then
        TriggerServerEvent('et-houses:server:viewHouse', ClosestHouse)
    end
end)

RegisterNetEvent('et-houses:client:EnterHouse', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
...
RegisterNetEvent('et-houses:client:RequestRing', function()
    if ClosestHouse ~= nil then
        TriggerServerEvent('et-houses:server:RingDoor', ClosestHouse)
    end
end)

AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent('et-houses:server:setHouses')
    SetClosestHouse()
    TriggerEvent('et-houses:client:setupHouseBlips')
    if Config.UnownedBlips then TriggerEvent('et-houses:client:setupHouseBlips2') end
    Wait(100)
    TriggerEvent('et-garages:client:setHouseGarage', ClosestHouse, HasHouseKey)
    TriggerServerEvent("et-houses:server:setHouses")
end)
...
RegisterNetEvent('et-houses:client:lockHouse', function(bool, house)
    Config.Houses[house].locked = bool
end)

RegisterNetEvent('et-houses:client:createHouses', function(price, tier)
    ...
    street = street:gsub("%-", " ")
    TriggerServerEvent('et-houses:server:addNewHouse', street, coords, price, tier)
    if Config.UnownedBlips then TriggerServerEvent('et-houses:server:createBlip') end
end)

RegisterNetEvent('et-houses:client:addGarage', function()
    if ClosestHouse ~= nil then
        local pos = GetEntityCoords(PlayerPedId())
        local heading = GetEntityHeading(PlayerPedId())
        local coords = {
        ...
        TriggerServerEvent('et-houses:server:addGarage', ClosestHouse, coords)
    else
        QBCore.Functions.Notify(Lang:t("error.no_house"), "error")
    end
end)

RegisterNetEvent('et-houses:client:toggleDoorlock', function()
    local pos = GetEntityCoords(PlayerPedId())
    local dist = #(pos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
    if dist <= 1.5 then
        if HasHouseKey then
            if Config.Houses[ClosestHouse].locked then
                TriggerServerEvent('et-houses:server:lockHouse', false, ClosestHouse)
                QBCore.Functions.Notify(Lang:t("success.unlocked"), "success", 2500)
            else
                TriggerServerEvent('et-houses:server:lockHouse', true, ClosestHouse)
...
RegisterNetEvent('et-houses:client:RingDoor', function(player, house)
...
RegisterNetEvent('et-houses:client:giveHouseKey', function()
    ...
    if player ~= -1 and distance < 2.5 and ClosestHouse ~= nil then
        ...
        if housedist < 10 then
            TriggerServerEvent('et-houses:server:giveHouseKey', playerId, ClosestHouse)
        else
...
RegisterNetEvent('et-houses:client:removeHouseKey', function()
    if ClosestHouse ~= nil then
        local pedpos = GetEntityCoords(PlayerPedId())
        local housedist = #(pedpos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
        if housedist <= 5 then
            QBCore.Functions.TriggerCallback('et-houses:server:getHouseOwner', function(result)
...
RegisterNetEvent('et-houses:client:RevokeKey', function(cData)
    RemoveHouseKey(cData.citizenData)
end)

RegisterNetEvent('et-houses:client:refreshHouse', function()
    Wait(100)
    SetClosestHouse()
end)

RegisterNetEvent('et-houses:client:SpawnInApartment', function(house)
    local pos = GetEntityCoords(PlayerPedId())
...
RegisterNetEvent('et-houses:client:enterOwnedHouse', function(house)
...
RegisterNetEvent('et-houses:client:LastLocationHouse', function(houseId)
    QBCore.Functions.GetPlayerData(function(PlayerData)
		if PlayerData.metadata["injail"] == 0 then
...
RegisterNetEvent('et-houses:client:setupHouseBlips', function() -- Setup owned on load
    CreateThread(function()
        Wait(2000)
        if LocalPlayer.state['isLoggedIn'] then
            QBCore.Functions.TriggerCallback('et-houses:server:getOwnedHouses', function(ownedHouses)
                if ownedHouses then
...
RegisterNetEvent('et-houses:client:setupHouseBlips2', function() -- Setup unowned on load
...
RegisterNetEvent('et-houses:client:createBlip', function(coords) -- Create unowned on command
...
RegisterNetEvent('et-houses:client:refreshBlips', function() -- Refresh unowned on buy
    for _, v in pairs(UnownedHouseBlips) do RemoveBlip(v) end
    Wait(250)
    TriggerEvent('et-houses:client:setupHouseBlips2')
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('et-houses:client:SetClosestHouse', function()
    SetClosestHouse()
end)

RegisterNetEvent('et-houses:client:viewHouse', function(houseprice, brokerfee, bankfee, taxes, firstname, lastname)
...
RegisterNetEvent('et-houses:client:setLocation', function(cData)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    ...
    if IsInside then
        if HasHouseKey then
            if cData.id == 'setstash' then
                TriggerServerEvent('et-houses:server:setLocation', coords, ClosestHouse, 1)
            elseif cData.id == 'setoutift' then
                TriggerServerEvent('et-houses:server:setLocation', coords, ClosestHouse, 2)
            elseif cData.id == 'setlogout' then
                TriggerServerEvent('et-houses:server:setLocation', coords, ClosestHouse, 3)
            end
        else
            QBCore.Functions.Notify(Lang:t("error.not_owner"), "error")
        end
...
RegisterNetEvent('et-houses:client:refreshLocations', function(house, location, type)
...
RegisterNetEvent('et-houses:client:HomeInvasion', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
                        ...
                        if not Config.Houses[ClosestHouse].IsRaming then
                            ...
                            Skillbar.Start({
                                ...
                                if RamsDone + 1 >= Config.RamsNeeded then
                                    ...
                                    TriggerServerEvent('et-houses:server:SetHouseRammed', true, ClosestHouse)
                                    TriggerServerEvent('et-houses:server:SetRamState', false, ClosestHouse)
                            ...
                            TriggerServerEvent('et-houses:server:SetRamState', true, ClosestHouse)
                        else
...
RegisterNetEvent('et-houses:client:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('et-houses:client:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('et-houses:client:ResetHouse', function()
    if ClosestHouse ~= nil then
        if Config.Houses[ClosestHouse].IsRammed == nil then
            Config.Houses[ClosestHouse].IsRammed = false
            TriggerServerEvent('et-houses:server:SetHouseRammed', false, ClosestHouse)
            TriggerServerEvent('et-houses:server:SetRamState', false, ClosestHouse)
        end
...
RegisterNetEvent('et-houses:client:ExitOwnedHouse', function()
...
RegisterNetEvent('et-houses:client:FrontDoorCam', function()
    local door = vector3(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z)
    if CheckDistance(door, 1.5) then
...
RegisterNetEvent('et-houses:client:AnswerDoorbell', function()
    ...
    if CheckDistance(door, 1.5) and CurrentDoorBell ~= 0 then
        TriggerServerEvent("et-houses:server:OpenDoor", CurrentDoorBell, ClosestHouse)
        CurrentDoorBell = 0
    end
end)

RegisterNetEvent('et-houses:client:OpenStash', function()
...
RegisterNetEvent('et-houses:client:ChangeCharacter', function()
    ...
    if CheckDistance(stashLoc, 1.5) then
        ...
        exports['et-interior']:DespawnInterior(houseObj, function()
            ...
            InOwnedHouse = false
            IsInside = false
            TriggerServerEvent('et-houses:server:LogoutLocation')
        end)
    end
end)

RegisterNetEvent('et-houses:client:ChangeOutfit', function()
...
RegisterNetEvent('et-houses:client:ViewHouse', function()
...
RegisterNetEvent('et-houses:client:KeyholderOptions', function(cData)
    optionMenu(cData.citizenData)
end)

RegisterNetEvent('et-house:client:RefreshHouseTargets', function ()
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)
...
RegisterNUICallback('HasEnoughMoney', function(cData, cb)
    QBCore.Functions.TriggerCallback('et-houses:server:HasEnoughMoney', function(_)
        cb('ok')
...
RegisterNUICallback('buy', function(_, cb)
    ...
    if Config.UnownedBlips then TriggerEvent('et-houses:client:refreshBlips') end
    TriggerServerEvent('et-houses:server:buyHouse', ClosestHouse)
    cb("ok")
end)
        ...
        if IsInside then
            ...
            if isInsideStashTarget then
                ...
                if IsControlJustPressed(0, 38) then
                    TriggerEvent('et-houses:client:OpenStash')
                    exports['et-core']:HideText()
                end
            end
            ...
            if isInsideOutfitsTarget then
                wait = 0
                if IsControlJustPressed(0, 38) then
                    TriggerEvent('et-houses:client:ChangeOutfit')
                    exports['et-core']:HideText()
                end
            end
            ...
            if isInsiteCharactersTarget then
                wait = 0
                if IsControlJustPressed(0, 38) then
                    TriggerEvent('et-houses:client:ChangeCharacter')
                    exports['et-core']:HideText()
                end
            end
        end

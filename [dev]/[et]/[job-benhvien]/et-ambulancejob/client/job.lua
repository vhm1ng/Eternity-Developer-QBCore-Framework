...
function Status()
    if isStatusChecking then
        ...
        exports['et-menu']:openMenu(statusMenu)
    end
end
...
RegisterNetEvent('hospital:client:RevivePlayer', function()
    if onDuty then
        ...
        QBCore.Functions.Notify("Bạn không có trong ca trực!", "error")
    end
end)
        ...
        while check do
            if IsControlJustPressed(0, 38) then
                ...
                elseif variable == "storeheli" then
                    TriggerEvent('et-ambulancejob:storeheli')
                elseif variable == "takeheli" then
                    TriggerEvent('et-ambulancejob:pullheli')
...
RegisterNetEvent('et-ambulancejob:stash', function()
    if onDuty then
...
RegisterNetEvent('et-ambulancejob:elevator_main', function()
    local ped = PlayerPedId()
...
CreateThread(function()
    for k, v in pairs(Config.Locations["vehicle"]) do
        ...
        boxZone:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" and onDuty then
                exports['et-core']:DrawText(Lang:t('text.veh_button'), 'left')
                EMSVehicle(k)
            else
                CheckVehicle = false
    ...
    for k, v in pairs(Config.Locations["helicopter"]) do
        ...
        boxZone:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" and onDuty then
                exports['et-core']:DrawText(Lang:t('text.heli_button'), 'left')
                EMSHelicopter(k)
            else
                CheckHeli = false
                exports['et-core']:HideText()
...
if Config.UseTarget then
    CreateThread(function()
        for k, v in pairs(Config.Locations["duty"]) do
            exports['et-target']:AddBoxZone("duty" .. k, vector3(v.x, v.y, v.z), 1.5, 1, {
                name = "duty" .. k,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
        ...
        for k, v in pairs(Config.Locations["stash"]) do
            exports['et-target']:AddBoxZone("stash" .. k, vector3(v.x, v.y, v.z), 1, 1, {
                name = "stash" .. k,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
        ...
        for k, v in pairs(Config.Locations["roof"]) do
            exports['et-target']:AddBoxZone("roof" .. k, vector3(v.x, v.y, v.z), 2, 2, {
                name = "roof" .. k,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
                ...
                options = {
                    {
                        type = "client",
                        event = "et-ambulancejob:elevator_roof",
        ...
        for k, v in pairs(Config.Locations["main"]) do
            exports['et-target']:AddBoxZone("main" .. k, vector3(v.x, v.y, v.z), 1.5, 1.5, {
                name = "main" .. k,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
                ...
                options = {
                    {
                        type = "client",
                        event = "et-ambulancejob:elevator_main",
                        icon = "fas fa-hand-point-up",
                        label = "Take Elevator",
                        job = "ambulance"
                    },
    ...
    CreateThread(function()
        ...
        signCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if not onDuty then
                    exports['et-core']:DrawText(Lang:t('text.onduty_button'), 'left')
                    EMSControls("sign")
                else
                    exports['et-core']:DrawText(Lang:t('text.offduty_button'), 'left')
                    EMSControls("sign")
                end
            else
                check = false
        ...
        armoryCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    exports['et-core']:DrawText(Lang:t('text.armory_button'), 'left')
                    EMSControls("armory")
                end
            else
                check = false
...
---------------------------
        ...
        roofCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    exports['et-core']:DrawText(Lang:t('text.elevator_main'), 'left')
                    EMSControls("main")
                else
                    exports['et-core']:DrawText(Lang:t('error.not_ems'), 'left')
                end
        ...
        mainCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    exports['et-core']:DrawText(Lang:t('text.elevator_roof'), 'left')
                    EMSControls("roof")
                else
                    exports['et-core']:DrawText(Lang:t('error.not_ems'), 'left')
                end

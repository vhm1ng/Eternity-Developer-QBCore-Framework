...
local function GetDeliveryLocation()
    ...
    PolyZone:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inRange = true
            exports["et-core"]:DrawText(Lang:t('info.busstop_text'), 'rgb(220, 20, 60)')
            CreateThread(function()
                repeat
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        resetNpcTask()
                        nextStop()
                        TriggerEvent('et-busjob:client:DoBusNpc')
...
local function busGarage()
    ...
    for _, v in pairs(Config.AllowedVehicles) do
        vehicleMenu[#vehicleMenu + 1] = {
            header = v.label,
            params = {
                event = "et-busjob:client:TakeVehicle",
                args = {
                    model = v.model
                }
            }
...
RegisterNetEvent("et-busjob:client:TakeVehicle", function(data)
...
RegisterNetEvent('et-busjob:client:DoBusNpc', function()
                    ...
                    CreateThread(function()
                        repeat
                            ...
                            if IsControlJustPressed(0, 38) then
                                ...
                                TriggerServerEvent('et-busjob:server:NpcPay')
                                exports["et-core"]:HideText()
                                PolyZone:destroy()
                                break
                            end
            ...
            if isPointInside then
                ...
                CreateThread(function()
                    repeat
                        Wait(5)
                        if not inVeh then
                            exports["et-core"]:DrawText(Lang:t('info.busstop_text'), 'left')
                            ...
                            exports["et-core"]:DrawText(Lang:t('info.bus_stop_work'), 'left')
                            if IsControlJustReleased(0, 38) then

...
RegisterNetEvent('et-kidnapping:client:SetKidnapping', function(bool)
    isKidnapping = bool
end)

RegisterNetEvent('et-trunk:client:KidnapTrunk', function()
    ...
    if distance ~= -1 and distance < 2 then
        if isKidnapping then
            ...
            if closestVehicle ~= 0 then
                ...
                Wait(50)
                TriggerServerEvent("et-trunk:server:KidnapTrunk", GetPlayerServerId(closestPlayer), closestVehicle)
            end
        else
...
RegisterNetEvent('et-trunk:client:KidnapGetIn', function(veh)
    ...
    if Config.TrunkClasses[vehClass].allowed then
        QBCore.Functions.TriggerCallback('et-trunk:server:getTrunkBusy', function(isBusy)
                    ...
                    if not isBusy then
                        if not isKidnapped then
                            if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                TriggerServerEvent('et-trunk:server:setTrunkBusy', plate, true)
                                inTrunk = true
                                Wait(500)
                            ...
                            if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                                ...
                                ClearPedTasks(ped)
                                inTrunk = false
                                TriggerServerEvent('et-smallresources:trunk:server:setTrunkBusy', plate, nil)
...
RegisterNetEvent('et-trunk:client:GetIn', function()
    local ped = PlayerPedId()
    local closestVehicle = getNearestVeh()
    if closestVehicle ~= 0 then
        local vehClass = GetVehicleClass(closestVehicle)
                ...
                if DoesEntityExist(vehicle) then
                    ...
                    if IsControlJustPressed(0, 38) then
                        if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                            ...
                            ClearPedTasks(ped)
                            inTrunk = false
                            TriggerServerEvent('et-trunk:server:setTrunkBusy', plate, false)
                            SetEntityCoords(ped, vehCoords.x, vehCoords.y, vehCoords.z)
                    ...
                    if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                        ...
                        if IsControlJustPressed(0, 47) then
                            if not IsVehicleSeatFree(vehicle, -1) then
                                TriggerServerEvent('et-radialmenu:trunk:server:Door', false, plate, 5)
                            else
                        ...
                        if IsControlJustPressed(0, 47) then
                            if not IsVehicleSeatFree(vehicle, -1) then
                                TriggerServerEvent('et-radialmenu:trunk:server:Door', true, plate, 5)
                            else

QBCore = exports['et-core']:GetCoreObject() -- Used Globally
inJail = false
jailTime = 0
currentJob = nil
CellsBlip = nil
...
local function ApplyClothes()
	...
	if DoesEntityExist(playerPed) then
		Citizen.CreateThread(function()
			...
			if gender == 0 then
				TriggerEvent('et-clothing:client:loadOutfit', Config.Uniforms.male)
			else
				TriggerEvent('et-clothing:client:loadOutfit', Config.Uniforms.female)
			end
		end)
	end
end
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
	...
	exports['et-target']:AddTargetEntity(freedom_ped, {
		options = {
			{
				type = "client",
	...
	exports['et-target']:AddTargetEntity(canteen_ped, {
		options = {
			{
				type = "client",
...
RegisterNetEvent('prison:client:Enter', function(time)
	...
	if invokingResource and invokingResource ~= 'et-policejob' and invokingResource ~= 'et-ambulancejob' and invokingResource ~= GetCurrentResourceName() then
	...
	if not Config.UseTarget then
		...
		freedom:onPlayerInOut(function(isPointInside)
			...
			if isPointInside then
				...
				exports['et-core']:DrawText('[E] Check Time', 'left')
			else
				exports['et-core']:HideText()
			end
		end)
		...
		canteen:onPlayerInOut(function(isPointInside)
			...
			if isPointInside then
				...
				exports['et-core']:DrawText('[E] Open Canteen', 'left')
			else
				exports['et-core']:HideText()
			end
		end)

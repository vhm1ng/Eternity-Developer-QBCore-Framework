...
local function callCops()
    ...
    if call == chance then
        TriggerServerEvent('et-diving:server:CallCops', coords)
    end
end
...
local function takeCoral(coral)
    ...
    QBCore.Functions.Progressbar("take_coral", Lang:t("info.collecting_coral"), times * 1000, false, true, {
        ...
        TriggerServerEvent('et-diving:server:TakeCoral', currentDivingLocation.area, coral, true)
        ClearPedTasks(ped)
...
local function setDivingLocation(divingLocation)
    if currentDivingLocation.area ~= 0 then
        for k in pairs(Config.CoralLocations[currentDivingLocation.area].coords.Coral) do
            if Config.UseTarget then
                exports['et-target']:RemoveZone(k)
            else
    ...
    for k, v in pairs(Config.CoralLocations[currentDivingLocation.area].coords.Coral) do
        if Config.UseTarget then
            exports['et-target']:AddBoxZone('diving_coral_zone_'..k, v.coords, v.length, v.width, {
            ...
            zones[k]:onPlayerInOut(function(inside)
                if inside then
                    currentArea = k
                    exports['et-core']:DrawText(Lang:t("info.collect_coral_dt"))
                else
                    currentArea = 0
...
local function sellCoral()
    ...
    QBCore.Functions.Progressbar("sell_coral_items", Lang:t("info.checking_pockets"), math.random(2000, 4000), false, true, {}, {}, {}, {}, function() -- Done
        ...
        TriggerServerEvent('et-diving:server:SellCoral')
    ...
    for i = 1, #Config.SellLocations do
        ...
        if Config.UseTarget then
            ...
            zone:onPlayerInOut(function(inside)
                if inside then
                    inSellerZone = true
                    exports['et-core']:DrawText(Lang:t("info.sell_coral_dt"))
                else
                    inSellerZone = false
                    exports['et-core']:HideText()
                end
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.TriggerCallback('et-diving:server:GetDivingConfig', function(config, area)
...
RegisterNetEvent('et-diving:client:NewLocations', function()
    QBCore.Functions.TriggerCallback('et-diving:server:GetDivingConfig', function(config, area)
        Config.CoralLocations = config
        setDivingLocation(area)
    end)
...
RegisterNetEvent('et-diving:client:UpdateCoral', function(area, coral, bool)
    Config.CoralLocations[area].coords.Coral[coral].PickedUp = bool
end)

RegisterNetEvent('et-diving:client:CallCops', function(coords, msg)
...
RegisterNetEvent("et-diving:client:setoxygenlevel", function()
    if oxgenlevell == 0 then
       ...
       TriggerServerEvent('et-diving:server:removeItemAfterFill')
    else
...
RegisterNetEvent('et-diving:client:UseGear', function()
    local ped = PlayerPedId()

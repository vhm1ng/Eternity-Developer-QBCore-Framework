...
local function FormatWeaponAttachments(itemdata)
    ...
    if itemdata.info.attachments ~= nil and next(itemdata.info.attachments) ~= nil then
        for _, v in pairs(itemdata.info.attachments) do
            if exports['et-weapons']:getConfigWeaponAttachments(itemdata.name) then
                for key, value in pairs(exports['et-weapons']:getConfigWeaponAttachments(itemdata.name)) do
...
local function CreateItemDrop(index)
	...
	if Config.UseTarget then
		exports['et-target']:AddTargetEntity(dropItem, {
			options = {
				{
...
RegisterNetEvent("et-inventory:client:closeinv", function()
    closeInventory()
end)
...
RegisterNetEvent('et-inventory:client:giveAnim', function()
...
RegisterNUICallback('RemoveAttachment', function(data, cb)
    ...
    local Attachment = exports['et-weapons']:getConfigWeaponAttachments(WeaponData.name:upper())[data.AttachmentData.attachment]
    QBCore.Functions.TriggerCallback('weapons:server:RemoveAttachment', function(NewAttachments)
        if NewAttachments ~= false then
            ...
            for _, v in pairs(NewAttachments) do
                for _, pew in pairs(exports['et-weapons']:getConfigWeaponAttachments(WeaponData.name:upper())) do
...
CreateThread(function()
    if Config.UseTarget then
        exports['et-target']:AddTargetModel(Config.VendingObjects, {
            options = {
                {
...
CreateThread(function()
    if Config.UseTarget then
        exports['et-target']:AddTargetModel(Config.CraftingObject, {
            options = {
                {

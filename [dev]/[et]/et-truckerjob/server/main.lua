...
RegisterNetEvent('et-trucker:server:DoBail', function(bool, vehInfo)
    local src = source
    ...
    if bool then
        if Player.PlayerData.money.cash >= Config.TruckerJobTruckDeposit then
            ...
            TriggerClientEvent('et-trucker:client:SpawnVehicle', src, vehInfo)
...
RegisterNetEvent('et-trucker:server:01101110', function(drops)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('et-trucker:server:nano', function()

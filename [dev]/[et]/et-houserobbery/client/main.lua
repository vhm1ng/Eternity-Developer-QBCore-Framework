...
local function enterRobberyHouse(house)
    ...
    if Config.Houses[house]["tier"] == 1 then
        data = exports['et-interior']:CreateHouseRobbery(coords)
    end
    Wait(100)
    houseObj = data[1]
    POIOffsets = data[2]
...
local function lockpickFinish(success)
    if success then
        TriggerServerEvent('et-houserobbery:server:enterHouse', closestHouse)
        QBCore.Functions.Notify(Lang:t("success.worked"), "success", 2500)
    else
        if usingAdvanced then
            if math.random(1, 100) < 20 then
                TriggerServerEvent("et-houserobbery:server:removeAdvancedLockpick")
                TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items["advancedlockpick"], "remove")
            end
        else
            if math.random(1, 100) < 40 then
                TriggerServerEvent("et-houserobbery:server:removeLockpick")
...
local function searchCabin(cabin)
    ...
    LockpickDoorAnim()
    TriggerServerEvent('et-houserobbery:server:SetBusyState', cabin, currentHouse, true)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            openingDoor = false
            ClearPedTasks(PlayerPedId())
            TriggerServerEvent('et-houserobbery:server:searchCabin', cabin, currentHouse)
            Config.Houses[currentHouse]["furniture"][cabin]["searched"] = true
            TriggerServerEvent('et-houserobbery:server:SetBusyState', cabin, currentHouse, false)
            SucceededAttempts = 0
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.TriggerCallback('et-houserobbery:server:GetHouseConfig', function(HouseConfig)
        Config.Houses = HouseConfig
    end)
end)

RegisterNetEvent('et-houserobbery:client:ResetHouseState', function(house)
...
RegisterNetEvent('et-houserobbery:client:enterHouse', function(house)
    enterRobberyHouse(house)
end)

RegisterNetEvent('et-houserobbery:client:setHouseState', function(house, state)
    Config.Houses[house]["opened"] = state
end)

RegisterNetEvent('et-houserobbery:client:setCabinState', function(house, cabin, state)
    Config.Houses[house]["furniture"][cabin]["searched"] = state
end)

RegisterNetEvent('et-houserobbery:client:SetBusyState', function(cabin, house, bool)

...
local function ResetHouseStateTimer(house)
    ...
    SetTimeout(time, function()
        ...
        TriggerClientEvent('et-houserobbery:client:ResetHouseState', -1, house)
    end)
end

-- Callbacks

QBCore.Functions.CreateCallback('et-houserobbery:server:GetHouseConfig', function(_, cb)
    cb(Config.Houses)
end)

-- Events

RegisterNetEvent('et-houserobbery:server:SetBusyState', function(cabin, house, bool)
    Config.Houses[house]["furniture"][cabin]["isBusy"] = bool
    TriggerClientEvent('et-houserobbery:client:SetBusyState', -1, cabin, house, bool)
end)

RegisterNetEvent('et-houserobbery:server:enterHouse', function(house)
    local src = source
    if not Config.Houses[house]["opened"] then
        ResetHouseStateTimer(house)
        TriggerClientEvent('et-houserobbery:client:setHouseState', -1, house, true)
    end
    TriggerClientEvent('et-houserobbery:client:enterHouse', src, house)
    Config.Houses[house]["opened"] = true
end)

RegisterNetEvent('et-houserobbery:server:searchCabin', function(cabin, house)
    local src = source
    ...
    TriggerClientEvent('et-houserobbery:client:setCabinState', -1, house, cabin, true)
end)

RegisterNetEvent('et-houserobbery:server:removeAdvancedLockpick', function()
...
RegisterNetEvent('et-houserobbery:server:removeLockpick', function()
    local Player = QBCore.Functions.GetPlayer(source)

RegisterNetEvent('vhming-mayin:client:UseDocument', function(ItemData)
...
RegisterNetEvent('vhming-mayin:client:SpawnPrinter', function()
...
RegisterNUICallback('SaveDocument', function(data, cb)
    if data.url then
        TriggerServerEvent('vhming-mayin:server:SaveDocument', data.url)
    end
    cb('ok')
end)
...
RegisterNetEvent('vhming-mayin:printer',function()
    SendNUIMessage({
        action = "start"
    })
    SetNuiFocus(true, true)
...
if Config.UseTarget then
    CreateThread(function()
        exports['et-target']:AddTargetModel("prop_printer_01", {
            options = {
                {
                    event = 'vhming-mayin:printer',
                    type = 'client',
                    icon = "fa fa-print	",

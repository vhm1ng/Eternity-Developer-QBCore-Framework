...
if Config.RenewedPhonePayment then
	RegisterNetEvent('cdn-fuel:client:phone:PayForFuel', function(amount)
		...
		local success = exports['et-phone']:PhoneNotification(Lang:t("fuel_phone_header"), Lang:t("phone_notification")..total, 'fas fa-gas-pump', '#9f0e63', "NONE", 'fas fa-check-circle', 'fas fa-times-circle')
		if success then
...
RegisterNetEvent('cdn-fuel:jerrycan:refuelvehicle', function(data)
	...
	if NotElectric then
		...
		if Config.Ox.Input then
			...
			local refuel = exports['et-input']:ShowInput({
...
local function PoliceAlert(coords)
	...
	if chance < Config.SyphonPoliceCallChance then
		...
		elseif Config.SyphonDispatchSystem == "et-dispatch" then
			TriggerServerEvent('et-dispatch:911call', coords)
		elseif Config.SyphonDispatchSystem == "et-default" then
		...
		if HasSyphon then
			...
			if reason == "syphon" then
				if Config.Ox.Input then
					...
					local syphon = exports['et-input']:ShowInput({

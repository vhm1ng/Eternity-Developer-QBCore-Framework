...
RegisterNetEvent('et-trashsearch:server:searchTrash', function(data)
    local src = source
    ...
    TriggerClientEvent('et-trashsearch:client:searchTrash', src, binId)
end)
...
-- Register NetEvent et-trashsearch:server:searchedTrash
RegisterNetEvent('et-trashsearch:server:searchedTrash', function(binId)
    local src = source

    -- Check if player is legit
    if not isLegit(src) then
        TriggerClientEvent("et-anticheat:client:ban", src, "et-trashsearch -> Player " .. GetPlayerName(src) .. " tried to bypass cooldown")
        error("et-trashsearch -> Player tried to bypass cooldown")
    end

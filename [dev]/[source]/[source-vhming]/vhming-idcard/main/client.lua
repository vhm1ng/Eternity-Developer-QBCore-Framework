...
-- Events
RegisterNetEvent('vhming-idcard:client:sendData', function(playerData)
...
RegisterNetEvent('vhming-idcard:client:startAnim', function(cardType)
...
RegisterNetEvent('vhming-idcard:client:notifyOx', function(data)
    lib.notify({title = data.title, description = data.desc, icon = data.icon, iconColor = data.iconColor})
end)

-- Client Callback Register
lib.callback.register('vhming-idcard:client:callBack:getClosestPlayer', function()
    return GetPlayerServerId(lib.getClosestPlayer(GetEntityCoords(cache.ped), 2.0, false))
end)

lib.callback.register('vhming-idcard:client:callBack:getMugShot', function()

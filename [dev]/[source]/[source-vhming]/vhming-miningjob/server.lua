...
RegisterNetEvent('QBCore:Server:UpdateObject', function() if source ~= '' then return false end QBCore = exports['et-core']:GetCoreObject() end)

-- ĐỘ BỀN CUỐC

RegisterServerEvent('vhming-miningjob:Durability', function(hp, data)
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)

	if hp <= 0 then
        Player.Functions.RemoveItem('pickaxe', 1, data.slot)
        TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items["pickaxe"], "remove")
		TriggerClientEvent("QBCore:Notify", src, "Cuốc của bạn đã bị hỏng!", "error")
    else
...
---Crafting
RegisterServerEvent('vhming-miningjob:GetItem', function(data)
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	...
	TriggerClientEvent("vhming-miningjob:CraftMenu", src, data)
end)

RegisterServerEvent('vhming-miningjob:MineReward', function()
...
RegisterServerEvent('vhming-miningjob:CrackReward', function(cost)
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
--Stone Cracking Checking Triggers
--Command here to check if any stone is in inventory
RegisterServerEvent('vhming-miningjob:WashReward', function(cost)
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    Player.Functions.RemoveItem('stone', cost)
...
RegisterServerEvent('vhming-miningjob:PanReward', function()
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	for i = 1, math.random(1,3) do
...
RegisterNetEvent("vhming-miningjob:Selling", function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('vhming-miningjob:server:toggleItem', function(give, item, amount)
	local src = source
	if give == 0 or give == false then
		...
		else TriggerEvent("vhming-miningjob:server:DupeWarn", item, src) end -- if not boot the player
	else
...
RegisterNetEvent("vhming-miningjob:server:DupeWarn", function(item, newsrc)

...
$(document).ready(function() {
    ...
    $('select').formSelect();
    $('.datepicker').datepicker({format: "dd/mm/yyyy", yearRange: 95});
...
$(document).on("click", ".container #slider #selectcharacter", function (e) {
    ...
    if (selected != char && switching == false) {
        ...
        $.post("https://vhming-multicharacter/switch", JSON.stringify({ char: selected, citizenid: ele.attr("data-citizenid")}), function(data) {
            if (selected >= 0) {
                setTimeout(()=>{
...
$(document).on("click", "#createchar_button", function (e) {
    ...
    $.post("https://vhming-multicharacter/create", JSON.stringify({ char: createdCharacter})).then((res) => {
        if (res) {
            $("body").fadeOut(300);
...
$(document).on("click", "#spawnplayer_button", function (e) {
    if (!switching) {
        $.post("https://vhming-multicharacter/select", JSON.stringify({ citizenid: $(`.container #slider #selectcharacter[data-char='${char}']`).attr('data-citizenid')}));
        $("body").fadeOut(500);
...
$(document).on("input", "#deleteplayer_input", function (e) {
    if ($(this).val() == characters[char-1]["name"]) {
        ...
        $.post("https://vhming-multicharacter/delete", JSON.stringify({ citizenid: $(`.container #slider #selectcharacter[data-char='${char}']`).attr('data-citizenid')}))
        .then((res) => {
            if (res) {

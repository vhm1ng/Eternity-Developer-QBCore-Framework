...
RegisterServerEvent('tC-miningjob:Durability', function(hp, data)
	local src = source
...
---Crafting
RegisterServerEvent('tC-miningjob:GetItem', function(data)
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	...
	TriggerClientEvent("tC-miningjob:CraftMenu", src, data)
end)

RegisterServerEvent('tC-miningjob:MineReward', function()
...
RegisterServerEvent('tC-miningjob:CrackReward', function(cost)
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
--Stone Cracking Checking Triggers
--Command here to check if any stone is in inventory
RegisterServerEvent('tC-miningjob:WashReward', function(cost)
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    Player.Functions.RemoveItem('stone', cost)
...
RegisterServerEvent('tC-miningjob:PanReward', function()
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	for i = 1, math.random(1,3) do
...
RegisterNetEvent("tC-miningjob:Selling", function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('tC-miningjob:server:toggleItem', function(give, item, amount)
	local src = source
	if give == 0 or give == false then
		...
		else TriggerEvent("tC-miningjob:server:DupeWarn", item, src) end -- if not boot the player
	else
...
RegisterNetEvent("tC-miningjob:server:DupeWarn", function(item, newsrc)

...
-- Callbacks
QBCore.Functions.CreateCallback('tC-vehicleshop:server:getVehicles', function(source, cb)
    local src = source
...
RegisterNetEvent('tC-vehicleshop:server:deleteVehicle', function (netId)
    local vehicle = NetworkGetEntityFromNetworkId(netId)
    DeleteEntity(vehicle)
end)

-- Sync vehicle for other players
RegisterNetEvent('tC-vehicleshop:server:swapVehicle', function(data)
    local src = source
    TriggerClientEvent('tC-vehicleshop:client:swapVehicle', -1, data)
    Wait(1500)-- let new car spawn
    TriggerClientEvent('tC-vehicleshop:client:homeMenu', src)-- reopen main menu
end)

-- Send customer for test drive
RegisterNetEvent('tC-vehicleshop:server:customTestDrive', function(vehicle, playerid)
    local src = source
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target))) < 3 then
        TriggerClientEvent('tC-vehicleshop:client:customTestDrive', target, vehicle)
    else
...
RegisterNetEvent('tC-vehicleshop:server:buyShowroomVehicle', function(vehicle)
    local src = source
...
RegisterNetEvent('tC-vehicleshop:server:sellShowroomVehicle', function(data, playerid)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target.PlayerData.source))) < 3 then
        ...
        if cash >= tonumber(vehiclePrice) then
            ...
            TriggerClientEvent('tC-vehicleshop:client:buyShowroomVehicle', target.PlayerData.source, vehicle, plate)

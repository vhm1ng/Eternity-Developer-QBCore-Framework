...
-- Events
RegisterNetEvent('tC-idcard:client:sendData', function(playerData)
...
RegisterNetEvent('tC-idcard:client:startAnim', function(cardType)
...
RegisterNetEvent('tC-idcard:client:notifyOx', function(data)
    lib.notify({title = data.title, description = data.desc, icon = data.icon, iconColor = data.iconColor})
end)

-- Client Callback Register
lib.callback.register('tC-idcard:client:callBack:getClosestPlayer', function()
    return GetPlayerServerId(lib.getClosestPlayer(GetEntityCoords(cache.ped), 2.0, false))
end)

lib.callback.register('tC-idcard:client:callBack:getMugShot', function()

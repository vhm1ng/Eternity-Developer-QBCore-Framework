	...
	if Config.MakeStores then
		for k, v in pairs(Config.Stores) do
			...
			Targets["ChairStore"..k] =
				exports['et-target']:AddCircleZone("ChairStore"..k, v.info.coords.xyz, 1.2, { name="ChairStore"..k, debugPoly=Config.Debug, useZ=true, },
...
RegisterNetEvent("jim-chairs:Use", function(item) local ped = PlayerPedId()
	...
	if not haschairalready then haschairalready = true
		...
		local chairTable = {
			{ "likemod_freefirethrone_props", 0, 0.2, -0.58, 8.4, 0, 190.0 },
...
AddEventHandler('onResourceStop', function(r) if r ~= GetCurrentResourceName() then return end
	if GetResourceState("et-target") == "started" or GetResourceState("ox_target") == "started" then

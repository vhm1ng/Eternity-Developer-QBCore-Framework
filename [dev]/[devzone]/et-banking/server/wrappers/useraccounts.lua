        ...
        if not self.cardActive then
            ...
            MySQL.update('UPDATE bank_cards SET cardnumber = ?, cardPin = ?, cardDecrypted = ?, cardActive = ?, cardLocked = ?, cardType = ? WHERE citizenid = ? AND record_id = ?', {
                ...
                if self.source ~= -1 then
                    TriggerClientEvent('et-banking:client:newCardSuccess', self.source, cardNumber, friendlyName)
...
RegisterNetEvent('et-banking:server:registerCurrentAccount', function(cid)
...
RegisterNetEvent('et-banking:server:registerSavingsAccount', function(cid)

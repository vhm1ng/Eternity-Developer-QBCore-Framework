                ...
                if dist < 20 then
                    ...
                    if dist < 5 then
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            TriggerServerEvent('et-taxi:server:NpcPay', meterData.currentFare)
                            meterActive = false
                            SendNUIMessage({
                                action = "resetMeter"
                            })
...
function TaxiGarage()
    ...
    for _, v in pairs(Config.AllowedVehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            header = v.label,
            params = {
                event = "et-taxi:client:TakeVehicle",
                args = {
                    model = v.model
                }
            }
...
RegisterNetEvent("et-taxi:client:TakeVehicle", function(data)
...
RegisterNetEvent('et-taxi:client:toggleMeter', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('et-taxi:client:enableMeter', function()
    if meterIsOpen then
        SendNUIMessage({
            action = "toggleMeter"
        })
...
RegisterNetEvent('et-taxi:client:toggleMuis', function()
    Wait(400)
    if meterIsOpen then
...
RegisterNetEvent('et-taxijob:client:requestcab', function()
    TaxiGarage()
end)
        ...
        exports['et-target']:SpawnPed({
            ...
            target = {
                options = {
                    {
                        type = "client",
                        event = "et-taxijob:client:requestcab",
    ...
    zone:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside then
            if whitelistedVehicle() and not isInsidePickupZone and not NpcData.NpcTaken then
                ...
                exports['et-core']:DrawText(Lang:t("info.call_npc"), Config.DefaultTextLocation)
                callNpcPoly()
            end
        else
    ...
    delieveryZone:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside then
            if whitelistedVehicle() and not isInsideDropZone and NpcData.NpcTaken then
                ...
                exports['et-core']:DrawText(Lang:t("info.drop_off_npc"), Config.DefaultTextLocation)
                dropNpcPoly()
            end
        else
...
function setupCabParkingLocation()
    ...
    taxiParking:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside and not Notified and Config.UseTarget then
            if whitelistedVehicle() then
                exports['et-core']:DrawText(Lang:t("info.vehicle_parking"), Config.DefaultTextLocation)
                Notified = true
        ...
        if isPlayerInsideZone then
            if IsControlJustReleased(0, 38) then
                ...
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    ...
                    if meterIsOpen then
                        TriggerEvent('et-taxi:client:toggleMeter')
                        meterActive = false
                    end
...
-- switched boss menu from et-bossmenu to taxijob
CreateThread(function()
    while true do
        local sleep = 1000

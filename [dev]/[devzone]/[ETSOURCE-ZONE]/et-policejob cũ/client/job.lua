    ...
    if coords then
        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
            ...
            QBCore.Functions.TriggerCallback('et-garage:server:GetVehicleProperties', function(properties)
                ...
                exports['vhming-fuel']:SetFuel(veh, vehicle.fuel)
...
function MenuImpound(currentSelection)
    ...
    QBCore.Functions.TriggerCallback("police:GetImpoundedVehicles", function(result)
        ...
        if shouldContinue then
            ...
            exports['et-menu']:openMenu(impoundMenu)
        end
    end)
...
RegisterNetEvent('police:client:ImpoundVehicle', function(fullImpound, price)
    ...
    local totalFuel = exports['vhming-fuel']:GetFuel(vehicle)
...
RegisterNetEvent('police:client:EvidenceStashDrawer', function(data)
    ...
    if #(pos - takeLoc) <= 1.0 then
        local drawer = exports['et-input']:ShowInput({
...
RegisterNetEvent('et-police:client:openArmoury', function()
...
RegisterNetEvent('et-police:client:spawnHelicopter', function(k)
    ...
    CreateThread(function()
        while true do
            ...
            if inFingerprint and PlayerJob.name == "police" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("et-police:client:scanFingerPrint")
                    break
                end
            else
                break
    ...
    CreateThread(function()
        while true do
            ...
            if inAmoury and PlayerJob.name == "police" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("et-police:client:openArmoury")
                    break
                end
            else
                break
    ...
    CreateThread(function()
        while true do
            ...
            if inHelicopter and PlayerJob.name == "police" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("et-police:client:spawnHelicopter")
                    break
                end
            else
                break
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["duty"]) do
            exports['et-target']:AddBoxZone("PoliceDuty_"..k, vector3(v.x, v.y, v.z), 1, 1, {
                name = "PoliceDuty_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
        ...
        for k, v in pairs(Config.Locations["stash"]) do
            exports['et-target']:AddBoxZone("PoliceStash_"..k, vector3(v.x, v.y, v.z), 1.5, 1.5, {
                name = "PoliceStash_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
                ...
                options = {
                    {
                        type = "client",
                        event = "et-police:client:openStash",
                        icon = "fas fa-dungeon",
        ...
        for k, v in pairs(Config.Locations["trash"]) do
            exports['et-target']:AddBoxZone("PoliceTrash_"..k, vector3(v.x, v.y, v.z), 1, 1.75, {
                name = "PoliceTrash_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
                ...
                options = {
                    {
                        type = "client",
                        event = "et-police:client:openTrash",
                        icon = "fas fa-trash",
                        label = "Open Trash",
                        job = "police",
                    },
        ...
        for k, v in pairs(Config.Locations["fingerprint"]) do
            exports['et-target']:AddBoxZone("PoliceFingerprint_"..k, vector3(v.x, v.y, v.z), 2, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "et-police:client:scanFingerPrint",
        ...
        for k, v in pairs(Config.Locations["armory"]) do
            exports['et-target']:AddBoxZone("PoliceArmory_"..k, vector3(v.x, v.y, v.z), 5, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "et-police:client:openArmoury",
                        icon = "fas fa-swords",
                        label = "Open Armory",
                        job = "police",
                    },
    ...
    dutyCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            if not PlayerJob.onduty then
                exports['et-core']:DrawText(Lang:t('info.on_duty'),'left')
                dutylistener()
            else
                exports['et-core']:DrawText(Lang:t('info.off_duty'),'left')
                dutylistener()
            end
        else
            dutylisten = false
    ...
    stashCombo:onPlayerInOut(function(isPointInside, _, _)
        if isPointInside then
            ...
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                exports['et-core']:DrawText(Lang:t('info.stash_enter'), 'left')
                stash()
            end
        else
            inStash = false
    ...
    trashCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inTrash = true
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                exports['et-core']:DrawText(Lang:t('info.trash_enter'),'left')
                trash()
            end
        else
            inTrash = false
    ...
    fingerprintCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inFingerprint = true
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                exports['et-core']:DrawText(Lang:t('info.scan_fingerprint'),'left')
                fingerprint()
            end
        else
            inFingerprint = false
    ...
    armouryCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inAmoury = true
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                exports['et-core']:DrawText(Lang:t('info.enter_armory'),'left')
                armoury()
            end
        else
            inAmoury = false
...
CreateThread(function()
    ...
    helicopterCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['et-core']:HideText()
                    exports['et-core']:DrawText(Lang:t('info.store_heli'), 'left')
                    heli()
                else
                    exports['et-core']:DrawText(Lang:t('info.take_heli'), 'left')
                    heli()
                end
            end
        else
    ...
    impoundCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['et-core']:DrawText(Lang:t('info.impound_veh'), 'left')
                    impound()
                else
    ...
    garageCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['et-core']:DrawText(Lang:t('info.store_veh'), 'left')
		    garage()
                else
                    local currentSelection = 0

...
## General Setup
General setup is quite easy, delete your OLD et-phone
if you server havent been running previously then go ahead and run the provided SQL file in your database.

If your server has been running et-phone previously please update your sql while being carefull and take a backup so you have no data lost.
...
1. Head over to et-phone/server/employment.lua and change local FirstStart from false to true like shown below

```lua
    local FirstStart = true
```

2. Start the script and make sure it's fully done, it can take a while depending on your current playerbase (ensure et-phone in console or f8)

3. Head over to et-phone/server/employment.lua again and change the FirstStart to false
...
4. Headover to your et-core/server/commands.lua and find the follow command 'setjob'
    ...
    if Player then
        ...
        if jobInfo then
            if jobInfo["grades"][sgrade] then
                ...
                exports['et-phone']:hireUser(job, Player.PlayerData.citizenid, grade)
            else
...
QBCore.Commands.Add('removejob', 'Removes A Players Job (Admin Only)', { { name = 'id', help = 'Player ID' }, { name = 'job', help = 'Job name' } }, true, function(source, args)
    ...
    if Player then
        ...
        exports['et-phone']:fireUser(tostring(args[2]), Player.PlayerData.citizenid)
    else
        TriggerClientEvent('QBCore:Notify', source, Lang:t('error.not_online'), 'error')
    end
end, 'admin')
...
6. If you use et-cityhall then u have to Find ApplyJob in et-cityhall/server/main.lua and replace with this one
...
1. Head over to your et-core/server/Player.lua

...
local function notifyPlayer(src, message)
    ...
    TriggerClientEvent('et-phone:client:CustomNotification', src,
        "Employment",
        message,
...
RegisterNetEvent('et-phone:server:fireUser', function(Job, sCID)
    local src = source
    ...
    TriggerClientEvent("et-phone:client:JobsHandler", -1, Job, CachedJobs[Job].employees)
    ...
    if Player.PlayerData.source then
        TriggerClientEvent('et-phone:client:MyJobsHandler', Player.PlayerData.source, Job, nil, nil)
    end
end)

---- ** Can give any employee x amount of money out of the bank account, must be boss ** ----
RegisterNetEvent('et-phone:server:SendEmploymentPayment', function(Job, CID, amount)
    local src = source
    ...
    if Config.RenewedBanking then
        ...
        if not exports['et-management']:RemoveMoney(Job, amt) then return notifyPlayer(src, "Insufficient Funds...") end
    end
    Player.Functions.AddMoney('bank', amt)
end)

---- ** Player can hire someone aslong as they are boss within the group
RegisterNetEvent('et-phone:server:hireUser', function(Job, id, grade)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if hiredPlayer and CachedPlayers[CID] then
        ...
        TriggerClientEvent('et-phone:client:MyJobsHandler', hiredPlayer.PlayerData.source, Job, CachedPlayers[CID][Job], CachedJobs[Job].employees)
    end
end)

---- ** Handles the changing of someone grade within the job ** ----

RegisterNetEvent('et-phone:server:gradesHandler', function(Job, CID, grade)
    local src = source
    local srcPlayer = QBCore.Functions.GetPlayer(src)
    ...
    if Player and CachedPlayers[CID] then
        ...
        TriggerClientEvent('et-phone:client:MyJobsHandler', Player.PlayerData.source, Job, CachedPlayers[CID][Job], CachedJobs[Job].employees)
    end
end)


RegisterNetEvent('et-phone:server:clockOnDuty', function(Job)
    local src = source
    ...
    if CachedPlayers[CID][Job] and CachedJobs[Job].employees[CID] then
        ...
        TriggerClientEvent('et-phone:client:clearAppAlerts', src)
    end
end)

---- Gets the client side cache for players ----
QBCore.Functions.CreateCallback("et-phone:server:GetMyJobs", function(source, cb)
    if FirstStart then return end
...
RegisterNetEvent('et-phone:server:ChargeCustomer', function(id, amount, notes, job)
    ...
    TriggerEvent('et-phone:server:CreateInvoice', tonumber(id), src, amt)
end)
    ...
    if paid then
        if amount == bills[source].amount then
            ...
            if Config.RenewedBanking then
                ...
                exports['et-management']:AddMoney(bills[source].job, amount)
            end

...
-- exports['et-phone']:RemoveCrypto(Player, type, amount)
local function RemoveCrypto(src, type, amount)
    ...
    if (Crypto[type] - amount) >= 0 then
        ...
        TriggerClientEvent('et-phone:client:UpdateCrypto', src)
        return true
    else
        return false
    end
...
-- exports['et-phone']:hasEnough(Player, type, amount)
...
-- exports['et-phone']:AddCrypto(Player, type, amount)
...
RegisterNetEvent('et-phone:server:PurchaseCrypto', function(type, amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('et-phone:server:SellCrypto', function(type, amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player or not Player.PlayerData.metadata.crypto[type] then return end -- if the crypto dosnt exist
    local v = Config.CryptoCoins[GetConfig(type)]
...
RegisterNetEvent('et-phone:server:ExchangeCrypto', function(type, amount, stateid)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player.PlayerData.citizenid ~= Receiver.PlayerData.citizenid then
        if RemoveCrypto(src, type, amount) then
            ...
            TriggerClientEvent('et-phone:client:CustomNotification', Receiver.PlayerData.source,
                "WALLET",

...
local function MenuGarage(type, garage, indexgarage)
    ...
    exports['et-menu']:openMenu({
        ...
        },
        {
            ...
            params = {
                event = "et-garages:client:VehicleList",
                args = {
                    type = type,
                    garage = garage,
                    index = indexgarage,
    ...
    garageZones[type .. "_" .. index].zonecombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            if type == "in" then
                ...
                exports['et-core']:DrawText(text, 'left')
                InputIn = true
            elseif type == "out" then
                if garage.type == "house" then
            ...
            elseif type == "house" then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    ...
                    exports['et-core']:DrawText(Lang:t("info.car_e"), 'left')
                    InputOut = true
                end
            end
        else
...
RegisterNetEvent("et-garages:client:VehicleList", function(data)
...
RegisterNetEvent('et-garages:client:takeOutGarage', function(data)
    local type = data.type
    local vehicle = data.vehicle
    local garage = data.garage
    local index = data.index
    QBCore.Functions.TriggerCallback('et-garage:server:IsSpawnOk', function(spawn)
        if spawn then
            local location
            if type == "house" then
    ...
    if GetVehicleNumberOfPassengers(veh) == 0 then
        QBCore.Functions.TriggerCallback('et-garage:server:checkOwnership', function(owned)
            if owned then
                ...
                local totalFuel = exports['vhming-fuel']:GetFuel(veh)
                TriggerServerEvent("et-vehicletuning:server:SaveVehicleProps", QBCore.Functions.GetVehicleProperties(veh))
                TriggerServerEvent('et-garage:server:updateVehicle', 1, totalFuel, engineDamage, bodyDamage, plate, indexgarage, type, PlayerGang.name)
                ...
                if plate then
                    TriggerServerEvent('et-garages:server:UpdateOutsideVehicle', plate, nil)
                end
...
RegisterNetEvent('et-garages:client:TakeOutDepot', function(data)
    ...
    if vehicle.depotprice ~= 0 then
        ...
        TriggerEvent("et-garages:client:takeOutGarage", data)
    end
end)

...
RegisterCommand('atm', function(source)
    ...
    TriggerClientEvent('et-atms:client:loadATM', src, cards)
end)
end
-- Event

RegisterNetEvent('et-atms:server:enteratm',function ()
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    local visas = xPlayer.Functions.GetItemsByName('visa')
    local masters = xPlayer.Functions.GetItemsByName('mastercard')
...
RegisterNetEvent('et-atms:server:doAccountWithdraw', function(data)
    if data ~= nil then
        local src = source
        local xPlayer = QBCore.Functions.GetPlayer(src)
        ...
        if dailyWith < Config.DailyLimit then
            ...
            TriggerClientEvent('et-atms:client:updateBankInformation', src, banking)
        else
...
QBCore.Functions.CreateCallback('et-debitcard:server:requestCards', function(source, cb)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    local visas = xPlayer.Functions.GetItemsByName('visa')
    local masters = xPlayer.Functions.GetItemsByName('mastercard')
...
QBCore.Functions.CreateCallback('et-debitcard:server:deleteCard', function(source, cb, data)
...
QBCore.Functions.CreateCallback('et-atms:server:loadBankAccount', function(source, cb, cid, cardnumber)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)

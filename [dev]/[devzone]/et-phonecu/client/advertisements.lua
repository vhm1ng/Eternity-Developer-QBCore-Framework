...
RegisterNUICallback('PostAdvert', function(data, cb)
    ...
    TriggerServerEvent('et-phone:server:AddAdvert', data.message, url)
    cb("ok")
end)


RegisterNUICallback('FlagAdvert', function(data, cb)
    TriggerServerEvent('et-phone:server:flagAdvert', data.number)
    cb("ok")
end)
...
RegisterNetEvent('et-phone:client:UpdateAdverts', function(Adverts, LastAd, src)

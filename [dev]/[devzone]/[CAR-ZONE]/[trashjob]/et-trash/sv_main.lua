...
RegisterServerEvent("garbage:createGroupJob", function(groupID)
    ...
    if FindJobById(groupID) == 0 then 
        ...
        if DoesEntityExist(car) then
            ...
            local members = exports["et-groups"]:getGroupMembers(groupID)
            for i=1, #members do 
                TriggerClientEvent('garbage:spawnTrash', members[i], plate)
            ...
            exports["et-groups"]:setJobStatus(groupID, "GARBAGE RUN")
        end

        exports["et-groups"]:CreateBlipForGroup(groupID, "garbagePickup", {
            label = "Pickup", 
        ...
        print("không tìm thấy id nhóm nào trong công việc")
    end
end)
...
RegisterServerEvent("garbage:stopGroupJob", function(groupID)
    ...
    if #(truckCoords - Config.Blip) < 30 then
        ...
        exports["et-groups"]:RemoveBlipForGroup(groupID, "garbagePickup")
        local members = exports["et-groups"]:getGroupMembers(groupID)
        local payout = math.floor((Jobs[jobID]["totalCollected"] * 40) / exports["et-groups"]:getGroupSize(groupID) + 0.5)
        ...
        for i=1, #members do
            ...
            if payout > 0 then
                ...
                TriggerClientEvent("QBCore:Notify", members[i], "You got $"..payout.." cho việc lấy rác của bạn", "success")
            end
        end

        Jobs[jobID] = nil
        exports["et-groups"]:setJobStatus(groupID, "WAITING")
    else 
        TriggerClientEvent("QBCore:Notify", src, "Xe tải của bạn không ở bên trong cơ sở", "error")
    end
end)
...
RegisterServerEvent("garbage:updateBags", function(groupID)
    ...
    if Jobs[jobID]["bags"] >= Jobs[jobID]["pickupAmount"] then
        ...
        local members = exports["et-groups"]:getGroupMembers(groupID)
        for i=1, #members do
            TriggerClientEvent("QBCore:Notify", members[i], "Tất cả các túi được thu thập cho thùng rác này", "primary")

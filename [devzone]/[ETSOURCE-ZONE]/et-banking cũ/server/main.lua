...
--[[ -- Only used by the following "et-banking:initiateTransfer"
...
RegisterNetEvent('et-banking:initiateTransfer', function(_)
    --[[
    local _src = source
    ...
    if (checkAccount) then
        ...
        if receiptName ~= false or receiptName ~= nil then
            local userOnline = exports.et-base:checkOnline(cid)

            if userOnline ~= false then
                -- User is online so we can do a straght transfer
                local _targetUser = exports.et-base:Source(userOnline)
                if acType == "Current" then
                    ...
                    TriggerClientEvent('et-banking:openBankScreen', _src)
                    TriggerClientEvent('et-banking:successAlert', _src, 'You have sent a bank transfer to '..receiptName..' for the amount of $'..data.amount)
                else
        ...
        TriggerClientEvent('et-banking:transferError', _src, 'The account details entered could not be located.')
    end
]]
end)
...
QBCore.Functions.CreateCallback('et-banking:getBankingInformation', function(source, cb)
...
RegisterNetEvent('et-banking:createBankCard', function(pin)
    local src = source
    ...
    TriggerClientEvent('et-banking:openBankScreen', src)
    TriggerClientEvent('et-banking:successAlert', src, Lang:t('success.debit_card'))

    TriggerEvent('et-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** successfully ordered a debit card")
end)

RegisterNetEvent('et-banking:doQuickDeposit', function(amount)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    ...
    if tonumber(amount) <= currentCash then
        ...
        if bank then
            TriggerClientEvent('et-banking:openBankScreen', src)
            TriggerClientEvent('et-banking:successAlert', src, Lang:t('success.cash_deposit', {value = amount}))
            TriggerEvent('et-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a cash deposit of $"..amount.." successfully.")
        end
    end
end)

RegisterNetEvent('et-banking:toggleCard', function(toggle)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('et-banking:doQuickWithdraw', function(amount, _)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if cash then
            TriggerClientEvent('et-banking:openBankScreen', src)
            TriggerClientEvent('et-banking:successAlert', src, Lang:t('success.cash_withdrawal', {value = amount}))
            TriggerEvent('et-log:server:CreateLog', 'banking', 'Banking', 'red', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a cash withdrawal of $"..amount.." successfully.")
        end
    end
end)

RegisterNetEvent('et-banking:updatePin', function(currentBankCard, newPin)
    if newPin ~= nil then
        local src = source
        local xPlayer = QBCore.Functions.GetPlayer(src)
        while xPlayer == nil do Wait(0) end
        ...
        MySQL.update('UPDATE bank_cards SET cardPin = ? WHERE record_id = ?', {
            ...
            if result == 1 then
                TriggerClientEvent('et-banking:openBankScreen', src)
                TriggerClientEvent('et-banking:successAlert', src, Lang:t('success.updated_pin'))
            else
...
RegisterNetEvent('et-banking:savingsDeposit', function(amount)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentBank then
        ...
        TriggerClientEvent('et-banking:openBankScreen', src)
        TriggerClientEvent('et-banking:successAlert', src, Lang:t('success.savings_deposit', {value = tostring(amount)}))
        TriggerEvent('et-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a savings deposit of $"..tostring(amount).." successfully..")
    end
end)

RegisterNetEvent('et-banking:savingsWithdraw', function(amount)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentSavings then
        ...
        while savings == nil do Wait(0) end
        TriggerClientEvent('et-banking:openBankScreen', src)
        TriggerClientEvent('et-banking:successAlert', src, Lang:t('success.savings_withdrawal', {value = tostring(amount)}))
        TriggerEvent('et-log:server:CreateLog', 'banking', 'Banking', 'red', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a savings withdrawal of $"..tostring(amount).." successfully.")
    end
end)

RegisterNetEvent('et-banking:createSavingsAccount', function()
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    local success = createSavingsAccount(xPlayer.PlayerData.citizenid)
    repeat Wait(0) until success ~= nil
    TriggerClientEvent('et-banking:openBankScreen', src)
    TriggerClientEvent('et-banking:successAlert', src, Lang:t('success.opened_savings'))
    TriggerEvent('et-log:server:CreateLog', 'banking', 'Banking', "lightgreen", "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** opened a savings account")
end)

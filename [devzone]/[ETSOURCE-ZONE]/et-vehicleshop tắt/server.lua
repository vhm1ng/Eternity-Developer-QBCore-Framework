...
RegisterNetEvent('et-vehicleshop:server:addPlayer', function(citizenid)
    financetimer[citizenid] = os.time()
end)

-- Deduct stored game time from player on logout
RegisterNetEvent('et-vehicleshop:server:removePlayer', function(citizenid)
...
-- Callbacks
QBCore.Functions.CreateCallback('et-vehicleshop:server:getVehicles', function(source, cb)
    local src = source
...
RegisterNetEvent('et-vehicleshop:server:deleteVehicle', function (netId)
    local vehicle = NetworkGetEntityFromNetworkId(netId)
    DeleteEntity(vehicle)
end)

-- Sync vehicle for other players
RegisterNetEvent('et-vehicleshop:server:swapVehicle', function(data)
    local src = source
    TriggerClientEvent('et-vehicleshop:client:swapVehicle', -1, data)
    Wait(1500)-- let new car spawn
    TriggerClientEvent('et-vehicleshop:client:homeMenu', src)-- reopen main menu
end)

-- Send customer for test drive
RegisterNetEvent('et-vehicleshop:server:customTestDrive', function(vehicle, playerid)
    local src = source
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target))) < 3 then
        TriggerClientEvent('et-vehicleshop:client:customTestDrive', target, vehicle)
    else
...
RegisterNetEvent('et-vehicleshop:server:financePayment', function(paymentAmount, vehData)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('et-vehicleshop:server:financePaymentFull', function(data)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    local cash = player.PlayerData.money['cash']
    local bank = player.PlayerData.money['bank']
...
RegisterNetEvent('et-vehicleshop:server:buyShowroomVehicle', function(vehicle)
    local src = source
    ...
    if cash > tonumber(vehiclePrice) then
        ...
        TriggerClientEvent('et-vehicleshop:client:buyShowroomVehicle', src, vehicle, plate)
...
RegisterNetEvent('et-vehicleshop:server:financeVehicle', function(downPayment, paymentAmount, vehicle)
    local src = source
...
RegisterNetEvent('et-vehicleshop:server:sellShowroomVehicle', function(data, playerid)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target.PlayerData.source))) < 3 then
        ...
        if cash >= tonumber(vehiclePrice) then
            ...
            TriggerClientEvent('et-vehicleshop:client:buyShowroomVehicle', target.PlayerData.source, vehicle, plate)
...
RegisterNetEvent('et-vehicleshop:server:sellfinanceVehicle', function(downPayment, paymentAmount, vehicle, playerid)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    local target = QBCore.Functions.GetPlayer(tonumber(playerid))
...
RegisterNetEvent('et-vehicleshop:server:checkFinance', function()
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
...
QBCore.Commands.Add('transfervehicle', Lang:t('general.command_transfervehicle'), {{name = 'ID', help = Lang:t('general.command_transfervehicle_help')}, {name = 'amount', help = Lang:t('general.command_transfervehicle_amount')}}, false, function(source, args)
    ...
    if not sellAmount then
        ...
        TriggerClientEvent('vehiclekeys:client:SetOwner2', buyerId, plate)

...
QBCore.Functions.CreateCallback('et-customs:server:GetLocations', function(_, cb)
    cb(Config.Locations)
end)
...
RegisterNetEvent('et-customs:server:attemptPurchase', function(type, upgradeLevel, location)
    local source = source
    ...
    if paidBySociety then
        if exports['et-management']:GetAccount(job) >= price then
            exports['et-management']:RemoveMoney(job, price)
        else
            paidBySociety = false
    ...
    if balance >= price or paidBySociety then
        ...
        if jobRestricted and job ~= 'mechanic' then
            exports['et-management']:AddMoney("mechanic", price)
        end
        TriggerClientEvent('et-customs:client:purchaseSuccessful', source)
    else
        TriggerClientEvent('et-customs:client:purchaseFailed', source)
    end
end)

RegisterNetEvent('et-customs:server:updateRepairCost', function(cost)
    local source = source
    RepairCosts[source] = cost
end)

RegisterNetEvent("et-customs:server:updateVehicle", function(myCar)
...
-- TriggerEvent('et-customs:server:UpdateLocation', 'Hayes', 'settings', 'enabled', test)

RegisterNetEvent('et-customs:server:UpdateLocation', function(location, type, key, value)
    local source = source
    if not QBCore.Functions.HasPermission(source, 'god') then return CancelEvent() end
    Config.Locations[location][type][key] = value
    TriggerClientEvent('et-customs:client:UpdateLocation', -1, location, type, key, value)
end)

-- name, help, args, argsrequired, cb, perms
QBCore.Commands.Add('customs', 'Open customs (admin only)', {}, false, function(source)
    local ped = GetPlayerPed(source)
    TriggerClientEvent('et-customs:client:EnterCustoms', source, {

...
local function saveVehicle()
    ...
    TriggerServerEvent('et-customs:server:updateVehicle', myCar)
end
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("et-customs:server:attemptPurchase", type, upgradeLevel, CustomsData.location)
...
function EnterLocation(override)
    ...
    QBCore.Functions.TriggerCallback('et-vehicletuning:server:IsMechanicAvailable', function(currentMechanics)
        ...
        if Config.UseRadial and radialMenuItemId then
            exports['et-radialmenu']:RemoveOption(radialMenuItemId)
...
function GetLocations()
    QBCore.Functions.TriggerCallback("et-customs:server:GetLocations", function(locations)
        ...
        if PlayerData.job.name ~= 'mechanic' and Config.DisableWhenMechanicsOnline and currentMechanics >= Config.MinOnlineMechanics then
            ...
            if Config.UseRadial then
                if not radialMenuItemId then
                    radialMenuItemId = exports['et-radialmenu']:AddOption({
                        id = 'customs',
                        title = 'Enter Customs',
                        icon = 'wrench',
                        type = 'client',
                        event = 'et-customs:client:EnterCustoms',
                        shouldClose = true
                    })
                end
            else
...
RegisterNetEvent('et-customs:client:UpdateLocation', function(location, type, key, value)
    Config.Locations[location][type][key] = value
end)

RegisterNetEvent("et-customs:client:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
    QBCore.Functions.Notify("Purchase Successful")
end)

RegisterNetEvent("et-customs:client:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    QBCore.Functions.Notify("Not enough money", "error")
end)

RegisterNetEvent('et-customs:client:EnterCustoms', function(override)

            ...
            for _, room in pairs(chatrooms) do
                ...
                if Player then
                    if Player.PlayerData.money.bank >= price then
                        ...
                        TriggerEvent('et-phone:server:sendNewEventMail', room.room_owner_id, {
                            room.room_owner_id,
...
QBCore.Functions.CreateCallback('et-phone:server:GetGroupChatMessages', function(_, cb, roomID)
...
QBCore.Functions.CreateCallback('et-phone:server:SearchGroupChatMessages', function(source, cb, roomID, searchTerm)
    local src = source
    ...
    if isOwnerOfRoom(Player.PlayerData.citizenid, roomID) or isMemberOfRoom(Player.PlayerData.citizenid, roomID) then
        ...
        TriggerClientEvent('et-phone:client:notification', src, 'Discord', 'You must be a member or room owner to search.')
        cb(false)
    end
end)

QBCore.Functions.CreateCallback('et-phone:server:GetPinnedMessages', function(source, cb, roomID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if isOwnerOfRoom(Player.PlayerData.citizenid, roomID) or isMemberOfRoom(Player.PlayerData.citizenid, roomID) then
        ...
        TriggerClientEvent('et-phone:client:notification', src, 'Discord', 'You must be a member or room owner to fetch that.')
        cb(false)
    end
end)


QBCore.Functions.CreateCallback('et-phone:server:TryPinCode', function(_, cb, pinCode, roomID)
...
QBCore.Functions.CreateCallback('et-phone:server:IsRoomOwner', function(source, cb, roomID)
...
RegisterNetEvent('et-phone:server:SendGroupChatMessage', function(messageData, systemMessage, roomID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if messageData and not systemMessage then
        if isOwnerOfRoom(Player.PlayerData.citizenid, roomID) or isMemberOfRoom(Player.PlayerData.citizenid, roomID) then
            ...
            TriggerClientEvent('et-phone:client:RefreshGroupChat', -1, src, messageData)
            TriggerEvent("et-log:server:CreateLog", "discord", "Message Posted (room: ".. messageData.room_id .. ", from: ".. Player.PlayerData.citizenid ..")", "blue", messageData.message)
        else
            TriggerClientEvent('et-phone:client:notification', src, 'Discord', 'You must be a member or room owner to send messages.')
        end
    else
        ...
        TriggerClientEvent('et-phone:client:RefreshGroupChat', -1, src, systemMessage)
    end
end)

QBCore.Functions.CreateCallback('et-phone:server:JoinGroupChat', function(source, cb, updatedRooms, roomID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not isMemberOfRoom(Player.PlayerData.citizenid, roomID) then
        ...
        TriggerClientEvent('et-phone:client:RefreshChatRooms', -1, ChatRooms)
        cb(true)
    end
end)

RegisterNetEvent('et-phone:server:LeaveGroupChat', function(updatedRooms, roomID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local members = {}
...
RegisterNetEvent('et-phone:server:ChangeRoomPin', function(updatedRooms, roomID, pin)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('et-phone:server:DeactivateRoom', function(updatedRooms, roomID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('et-phone:server:PurchaseRoom', function(source, cb, price, roomData)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.money.bank >= price then
        ...
        TriggerClientEvent('et-phone:client:RefreshChatRooms', -1, ChatRooms)
        TriggerEvent("et-log:server:CreateLog", "discord", "Channel Created:  ".. roomData.room_name .."(id: ".. roomID.. ", by: "..Player.PlayerData.citizenid..")", "blue")
		cb(true)
	else
		cb(false)
	end
...
RegisterNetEvent('et-phone:server:ToggleMessagePin', function(messageID, roomID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)

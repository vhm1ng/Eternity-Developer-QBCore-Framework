...
$(document).on('click', '.box-track', function(e){
    e.preventDefault()
    $.post("https://et-phone/gps-vehicle-garage", JSON.stringify({
        veh: veh,
    }));
});
...
$(document).on('click', '#garage-sellvehicle', function(e){
    ...
    if(price != "" && stateid != ""){
        $.post("https://et-phone/sellVehicle", JSON.stringify({
            plate: plate,
            id: stateid,
            price: price
        }));
    ...
    if (Vehicles != null) {
        $.each(Vehicles, function(i, vehicle){
            ...
            '<div class="garage-block">' +
                ...
                '<div class="garage-box" id="'+vehicle.plate+'"><span class="garage-box box-track" style="margin-left: 35%;">Định vị</span>' +
                // <span class="garage-box box-sellvehicle" style = "margin-left: 1.1vh;">SELL</span>
                '</div>' +
            '</div>' +
            '</div>';

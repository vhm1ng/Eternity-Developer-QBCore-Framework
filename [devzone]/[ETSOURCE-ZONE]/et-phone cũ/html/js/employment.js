...
$(document).on('click', '.employment-list', function(e){
    ...
    $.post('https://et-phone/GetEmployees', JSON.stringify({job: job}), function(data){
...
$(document).on('click', '#employment-job-extras-icon', function(e){
    ...
    $.post('https://et-phone/dutyStatus', JSON.stringify({}), function(data) {
        currentJob = data["job"]
        if (data["duty"]) {
            onDuty = true
        } else {
...
$(document).on('click', '#clock-in', function(e){
    ...
    $.post('https://et-phone/ClockIn', JSON.stringify({
        job: job
    }));
    closeDropDown()
});
...
$(document).on('click', '#hire-worker-submit', function(e){
    ...
    if(stateid !== "" && grade !== ""){
        ...
        $.post('https://et-phone/HireFucker', JSON.stringify({
            stateid: stateid,
            grade: grade,
            job: job,
        }));
...
$(document).on('click', '#employment-chargemf-submit', function(e){
    ...
    if(stateid != "" && amount != "" && note != ""){
        ...
        $.post('https://et-phone/ChargeMF', JSON.stringify({
            stateid: stateid,
            amount: amount,
            note: note,
            job: job,
...
$(document).on('click', '#send-employee-payment', function(e){
    ...
    if(amount != "" && note != ""){
        ...
        $.post('https://et-phone/SendEmployeePayment', JSON.stringify({
            cid: cid,
            job: job,
            amount: amount,
            note: note,
...
$(document).on('click', '#employment-remove-employee', function(e){
    ...
    $.post('https://et-phone/RemoveEmployee', JSON.stringify({
        cid: cid,
        job: job,
    }));
});
...
$(document).on('click', '#employment-changerole-submit', function(e){
    ...
    if(grade != ""){
        ...
        $.post('https://et-phone/ChangeRole', JSON.stringify({
            cid: cid,
            grade: grade,
            job: job
        }));

...
QB.Phone.Functions.SetupMails = function(Mails) {
    if (Mails !== null && Mails !== undefined) {
        ...
        if (Mails.length > 0) {
                ...
                if (JSON.stringify(mail.button) != "[]"){
                    var element = '<div class="mail" id="mail-'+mail.mailid+'" data-mailid="'+mail.mailid +'">'+
                        '<span class="mail-sender">From: '+mail.sender+'</span>' +
                            ...
                            '<div class="mail-block">' +
                                ...
                                '<div class="mail-box"><span class="mail-box mail-accept" style="margin-left: 4.0vh;">Đồng ý</span><span class="mail-box mail-delete" style = "margin-left: 1.1vh;">Xóa</span></div>' +
                            '</div>' +
                    ...
                    var element = '<div class="mail" id="mail-'+mail.mailid+'" data-mailid="'+mail.mailid +'">'+
                        '<span class="mail-sender">From: '+mail.sender+'</span>' +
                            ...
                            '<div class="mail-block">' +
                                '<div class="mail-message">'+mail.message+'</div>' +
                                '<div class="mail-box"><span class="mail-box mail-delete" style = "margin-left: 7.0vh;">Xóa</span></div>' +
                            '</div>' +
                            '<div class="mail-line"></div>' +
                            '<div class="mail-time">'+TimeAgo+'</div>' +
                        '</div>';
            ...
            $(".mail-list").html('<p class="nomails">Không có gì! <i class="fas fa-frown" id="mail-frown"></i></p>');
        }
...
$(document).on('click', '.mail-accept', function(e){
    ...
    $.post('https://et-phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '.mail-delete', function(e){
    e.preventDefault();
    var mailId = $(this).parent().parent().parent().data('mailid');
    $.post('https://et-phone/RemoveMail', JSON.stringify({
        mailId: mailId
    }));
});
